...
(async () => {
  ...
  const LINE = '+919000001111', CUST = '+919000002222';
  ...
  const unver = await deliver('20 bags of cement UNVERIFIED-CASE', 'UNVER');
  ...
  const offRes = await deliver('12 bags of cement OFFCASE please', 'OFF');
  ...
  ok(!(await waitForChit('OFFCASE', 2)), '★★★ OFF means OFF — no chit appeared without a person',
    'a chit was raised while the switch was off');
  ok((await pending()).some((c) => /OFFCASE/.test(c.raw_text || '')), '★ and it is waiting in Intake, as it always was');

  // ── 3. ON → the chit appears, and NOTHING here asked for it.
  await j('/api/channels/' + bind.id + '/auto-raise', { method: 'POST', token: tok, body: { on: true } });
  const onRes = await deliver('8 bags of cement and 15 kg nails ONCASE by friday', 'ON');
  ok(onRes.b && onRes.b.captured >= 1, 'the message was captured');
  const chit = await waitForChit('ONCASE', 60);
  ...
  if (chit) {
    ...
    ok(sum.via && /ONCASE/.test(sum.via.raw_excerpt || ''), '★ their own words are on it');
    ...
    ok(!(await pending()).some((c) => /ONCASE/.test(c.raw_text || '')), '★ and it left the intake queue');
  }

  // ── 4. A REDELIVERY must not mint a second chit. Meta retries; a retry is not a second order.
  const before = (await inbox()).length;
  await deliver('8 bags of cement and 15 kg nails ONCASE by friday', 'ON');   // same provider_msg_id
  await sleep(20000);
   ...
   */
  await deliver('ok thanks NOISECASE', 'NOISE');
  await sleep(30000);
  ok(!(await waitForChit('NOISECASE', 3)), '★★★ a message with no order in it did NOT become a chit');
  ok((await pending()).some((c) => /NOISECASE/.test(c.raw_text || '')), '★★ it is waiting in Intake for a person');
