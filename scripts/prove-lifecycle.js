...
// Proves:
//   A. ASSIMILATION — a minted chit's governance stamp carries BOTH facets: iso-9000@v1 (quality) + exim-policy@v1 (trade).
...
(async () => {
  ...
  chk('stamp assimilates ISO 9000 (quality)', stampStr.includes('iso-9000'), 'iso-9000 present');
  ...
  chk('both standards READ at runtime', checkedRefs.includes('iso-9000@v1') && checkedRefs.includes('exim-policy@v1'),
  ...
  chk('ISO 9000 (entity-scope) is read but NOT mis-applied to the chit',
    !((v1.j && v1.j.gaps) || []).some(g => String(g.standard).startsWith('iso-9000')), 'no false quality gaps');
