...
function decorate(lines, shelf) {
  ...
  return (Array.isArray(lines) ? lines : []).map((l) => {
    ...
    return Object.assign({}, l, {
      ...
      tax_source: r.source, hsn: l.hsn || d.hsn || d.hs_code || d.hsn_code || (r.hsn && r.hsn[0]) || null,
    });
  });
}
...
function invoiceFor(inp) {
  ...
  const det = tax.determine({
    ...
    lines: lines.map((l) => ({
      ...
      cess_rate: num(l.cess_rate) || 0, hsn: l.hsn || '',
    })),
...
function heads(inv) {
  ...
  return { taxable: r2(v.AssVal), cgst: r2(v.CgstVal), sgst: r2(v.SgstVal), igst: r2(v.IgstVal), cess: r2(v.CesVal),
           total: r2(v.TotInvVal), tax: r2((v.CgstVal || 0) + (v.SgstVal || 0) + (v.IgstVal || 0) + (v.CesVal || 0)) };
}
const zero = () => ({ taxable: 0, cgst: 0, sgst: 0, igst: 0, cess: 0, total: 0, tax: 0 });
...
module.exports = { stateOfGstin, partyOf, decorate, invoiceFor, heads, ledger, gstr1, gstr3b };
