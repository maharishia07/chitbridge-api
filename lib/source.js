...
async function registerSource({ source_key, title, facet, kind, template }) {
  ...
  return withTransaction(async (c) => {
    ...
    if (!ent.rows.length) {
      ent = await c.query(
        `INSERT INTO identities (bridge_id, display_name, email, identity_type, status, sealed, owner_scope, entity_kind)
         VALUES ($1,$2,$3,'entity','active',true,'platform','internal') RETURNING identity_id, bridge_id, display_name`,
