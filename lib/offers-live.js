  ...
  try {
    ...
    items.forEach((it, i) => {
      ...
      if (p && p.off > 0) {
        ...
        var promise = null; try { var src = offers.find(function (o) { return String(o.id) === String(p.offer_id); }); promise = (src && eng.promise) ? eng.promise(src, { now: new Date(), currency: entity.currency_code || 'INR', money: function (n) { return String(n); } }) : null; } catch (_) { promise = null; }
        ...
        var parts = (ev.adjustments || []).filter(function (a) { return a.scope === 'line' && String(a.target) === String(i) && Math.abs(Number(a.amount) || 0) > 0; })
          .map(function (a) { var src = offers.find(function (o) { return String(o.id) === String(a.offer_id); }); var pr = null; try { pr = (src && eng.promise) ? eng.promise(src, { now: new Date(), currency: entity.currency_code || 'INR', money: function (n) { return String(n); } }) : null; } catch (_) {}
    ...
    if (cartAdj.length) {
      /* the share is by LIST value (price × qty) — the same split the live cart's tax uses (cart.js money: orderOff × g / gross), so both sides agree to the paisa */
      const priced = []; items.forEach((it, i) => { if (lines.some((l) => l.key === String(i)) && Number.isFinite(Number(it.total))) priced.push({ it, i, pre: money.round2(Number(it.price) * Number(it.quantity)) }); });
      const base = priced.reduce((t, x) => t + x.pre, 0);
      cartAdj.forEach((a) => {
        const amt = money.round2(Math.abs(Number(a.amount) || 0)); if (!amt || base <= 0) return;
        var src = offers.find(function (o) { return String(o.id) === String(a.offer_id); }); var pr = null; try { pr = (src && eng.promise) ? eng.promise(src, { now: new Date(), currency: entity.currency_code || 'INR', money: function (n) { return String(n); } }) : null; } catch (_) {}
        let left = amt;
        priced.forEach((x, idx) => {
          const share = idx === priced.length - 1 ? money.round2(left) : money.round2(amt * x.pre / base); left = money.round2(left - share); if (share <= 0) return;
          const it = x.it, cur = it.offer || null;
          ...
          parts.push({ offer_id: a.offer_id || null, label: a.label || a.kind || 'offer', off: share, scope: 'cart', promise: pr || undefined });
        ...
        applied.push({ scope: a.scope || 'cart', label: a.label || a.kind, amount: amt, offer_id: a.offer_id || null, prorated: true });
      });
