...
async function applyLiveOffers(entity, items, total, deps) {
  try {
    ...
    /**
     * ⭐⭐ AND THE NETWORK'S, ON THE NETWORK'S PRODUCTS (2026-09-17). A member store's storefront now shows the offers its brand
     * released and the store took (catalogue-view → network-offers.forFinishes); the order must charge the same. Only lines
     * adopted from ANOTHER owner ask, keyed exactly as the storefront keyed them, so the promise and the invoice agree.
     */
    const netKey = {};
    const fins = [];
    items.forEach((it, i) => {
      const owner = it && it.kind === 'finish' && it.governed && it.governed.owner_entity_id;
      if (!owner || String(owner) === String(entity.identity_id)) return;
      const d = it.d || {};
      const nl = require('./network-offers');
      netKey[i] = nl.lineKey(it.source, { sku: d.sku, name: it.finish || it.name });
      fins.push({ source: it.source, owner_entity_id: owner, items: [{ name: it.finish || it.name, sku: d.sku || null }] });
    });
    if (fins.length) {
      const net = await require('./network-offers').forFinishes(entity.identity_id, fins).catch(() => []);
      offers.push(...net);
    }
    ...
    items.forEach((it, i) => {
      ...
      /* a network line has no item id of its own — it carries the key the storefront and the offers use, and its category NAME */
      const cats = [].concat(Array.isArray(d.categories) ? d.categories : [], netKey[i] && d.category ? [d.category] : []).map(String);
      lines.push({ key: String(i), item_id: netKey[i] || it.item_id || (it.ref && it.ref.item_id) || null, sku: it.sku || (d.sku || d.code) || (it.ref && it.ref.code) || null, excluded: Array.isArray(d.offers_excluded) ? d.offers_excluded.map(String) : [],
        categories: cats, qty: Number(it.quantity), unitPrice: Number(it.price) });
    });
...
/**
 * ⭐⭐⭐ [REV-02] "a bill the customer has paid is re-priced at today's offers when it syncs." `applyLiveOffers`
 * already leaves a line alone once it carries `.offer` — the gap was a line with NONE, which a later, unrelated
 * offer attaches itself to as if it had never been priced. `business_json.billed_at` is the tell: only
 * till.html/till.js ever set it (a real, already-printed counter bill or credit note), so its caller — a chit's
 * send route — checks THIS before deciding whether to call applyLiveOffers at all, not per line. Pulled out here,
 * beside the function it gates, so the rule has one name and one test instead of living as an inline `!!` at the
 * call site.
 */
function alreadyBilled(business_json) { return !!(business_json && business_json.billed_at); }
module.exports = { applyLiveOffers, alreadyBilled };
