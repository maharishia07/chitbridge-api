 ...
 * @param origin  { entity_id, definition_id, ref, sub_kind, note, subject, screen_code }
 * @param who     { id, name }
...
async function toOperator(kind, origin, who) {
  try {
    ...
    const out = await raiseticket.raise(
      ...
      who,
      { kind, ref: origin.ref, subject: origin.subject || origin.note, detail: origin.note,
        severity: origin.sub_kind, definition_id: origin.definition_id, screen_code: origin.screen_code });
    ...
    return { copied: !!out.raised, chit_id: out.chit_id, folder: out.folder,
             assigned: out.assigned, why: out.why || out.routed };
  } catch (e) {
