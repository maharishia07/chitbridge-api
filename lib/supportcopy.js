 ...
 *                `audience` is 'platform' (the rail is broken — it reaches us) or 'here' (my own business has
 *                a fault — it never leaves my entity). Default 'platform': every caller that shipped before
 *                today meant us, and a default that re-routed them inward would lose their tickets silently.
 * @param who     { id, name }
...
async function toOperator(kind, origin, who) {
  try {
    ...
    const out = await raiseticket.raise(
      ...
      who,
      { kind, audience: origin.audience, ref: origin.ref, subject: origin.subject || origin.note,
