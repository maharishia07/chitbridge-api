...
async function toOperator(kind, origin, who) {
  ...
  /**
   * ⭐ AND IT LANDS IN THE OPERATOR'S OWN QUEUE, not in a heap. b250 lets CBINC say where incidents go and who
   * owns them; without it every shop's fault arrives addressed to nobody, and the first week of real support
   * would bury the one that mattered.
   *
   * ⚠️⚠️ RESOLVED BEFORE THE TRANSACTION OPENS, NOT INSIDE IT. routeFor() opens its own withEntity, and
   * withEntity pins a transaction to ONE entity — nesting a second holds TWO connections for the length of the
   * write. routes/testing.js says exactly this about its own citation lookup, which is how I knew to check.
   * [[project-round-trip-cost]]
   *
   * ⚠️ Read with OUR entity: the shop's routing is about the shop's staff and has nothing to say about who
   * answers it here.
   */
  const route = await require('./workroute').routeFor(root, kind);
  ...
  try {
    const id = await withEntity(root, async (db) => {
      ...
      const rules = Object.assign({}, origin.rules || {}, {
        /* ⚠️ recorded on the copy rather than resolved at read time: who owned a fault WHEN IT ARRIVED is the
           fact a support report is judged on, and rota changes must not rewrite history. */
        assigned_to: route.assignee_actor_id || null,
