...
async function resolveEntityGovernance(entity_id, opts) {
  ...
  return cascade({ constKey, instKey, stamp, fresh,
                   resolved_from: stamp ? 'entity_stamp' : 'fallback',
                   fallback: !stamp });
}

/**
 * ── ⭐⭐⭐ THE SAME CASCADE, FROM AN INSTALLATION ALONE — this is what makes it work for CTP ─────────────────────
 *
 * Athi, 2026-09-16: *"verify the constitution matrix, so it resolves correctly here; if not, bring it to the
 * same place, so it works for CTP."*
 *
 * ⚠️⚠️ IT DID NOT WORK FOR CTP, AND IT FAILED SILENTLY. resolveEntityGovernance is keyed by a LOCAL entity and
 * reads `entity_governance` under RLS. A party arriving over the wire has no row here at all — so for a foreign
 * sender the stamp came back null and the cascade quietly resolved them as `base` @ `platform-0`, the way a
 * brand-new local shop would resolve. An Emirati supplier's chit would have been governed as a default Indian
 * one, with nothing on screen saying so. [[feedback-silence-is-the-bug]]
 *
 * ⭐ THE RULE ALREADY EXISTED — IN SQL. `identities_stamp_world()` (b254) stamps a new entity by reading
 * `installation.vertical_key` and using THAT as the constitution: installation → vertical → constitution. And a
 * remote peer has an `installation` row here (b257, `hosted_locally = false`) carrying exactly that column. So
 * a foreign party's governance is resolvable from `from.installation_key` alone, with no entity row and no
 * RLS context — this function is that rule in JavaScript, and `resolveEntityGovernance` now goes through the
 * same cascade. One cascade, two doors.
 *
 * ⚠️ AND IT SAYS WHERE ITS ANSWER CAME FROM. `resolved_from` is `entity_stamp` · `installation` · `fallback`,
 * and `fallback: true` means "I could not find you and this is the default" — a fact, printed, not a quiet
 * substitution.
 */
async function resolveInstallationGovernance(installation_key, opts) {
  const fresh = !!(opts && opts.fresh);
  const instKey = String(installation_key || '').trim() || 'platform-0';
  const inst = await readInstallation(instKey, fresh);
  /* the SAME rule as identities_stamp_world(): the installation's vertical IS its constitution */
  const constKey = (inst && inst.vertical_key) || 'base';
  return cascade({ constKey, instKey, stamp: null, fresh,
                   resolved_from: inst ? 'installation' : 'fallback',
                   fallback: !inst });
}

/** ⭐ ONE cascade. Everything below this line used to be the body of resolveEntityGovernance, unchanged. */
async function cascade({ constKey, instKey, stamp, fresh, resolved_from, fallback }) {
  ...
  return {
    constitution: versionref.format(constKey, (cons && cons.version) || (stamp && stamp.constitution_version)),
    installation: inst
    ...
    allowances,
    /* ⭐ WHERE THIS ANSWER CAME FROM — `entity_stamp` · `installation` · `fallback`. A caller that ignores
       `fallback: true` is choosing to treat a default as a fact; before 2026-09-16 it had no way to know. */
    resolved_from,
    fallback: !!fallback,
  };
}
...
module.exports = { resolveEntityGovernance, resolveInstallationGovernance, currencyRefusal };
