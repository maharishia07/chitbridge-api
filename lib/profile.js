...
async function saveVault(entity_id, v, opts) {
  ...
  try {
    let upi = null; for (const sec of vault.sections) for (const r of sec.rows) if (r.tag === 'upi_id' && isUpiId(r.value)) upi = { value: String(r.value).trim(), source: r.source || 'profile', as_of: r.as_of || null, rung: r.rung || 'declared' };
...
/**
 * ⭐ IS THIS A UPI ID WE MAY SHOW? A payee address goes on a QR a stranger scans, so it is validated in ONE place
 * and every surface asks here — the storefront, the counter, and whatever comes next.
 * ⚠️ Deliberately narrow: handle@bank, and nothing exotic. A permissive check on a payee address is not generosity.
 */
function isUpiId(v) { return !!v && /^[\w.\-]{2,}@[\w\-]{2,}$/.test(String(v)); }
/**
 * ⭐ THE PAYEE ADDRESSES A SHOP HAS DECLARED, as (scheme, value) — the same shape the jurisdiction work chose for
 * identifiers, because a payee address has the same problem: there is no universal one.
 * ⚠️ TODAY ONLY UPI IS MIRRORED onto the identities row, so today this returns at most one. It is written this way
 * so the second scheme is a row in a table, not a rewrite of every caller.
 */
const PAY_SCHEMES = {
  /* value: what the shop declares · encode: how a QR is made · countries: where it is offered at all */
  upi: { label: 'UPI', countries: ['IN'], encodes: 'upi-deeplink', valid: isUpiId,
         why: 'NPCI deep link. India only; the customer scans with any UPI app and the money moves bank to bank.' },
};
/** ⚠️ NOT a scheme we can encode — recorded, never generated. Kept here so the list of ways to be paid is one list. */
const PAY_RECORD_ONLY = {
  card:   { label: 'Card',   countries: null, why: 'the terminal takes it; the till records that it did' },
  wallet: { label: 'Wallet', countries: null, why: 'Apple Pay, Google Pay and the like ride a card rail through an acquirer' },
  cash:   { label: 'Cash',   countries: null, why: 'always' },
};
/** the mirrored copy on an identities row (policy_flags.profile_provenance.upi_id) → the id, or null */
function upiOf(policy_flags) {
  try {
    const v = policy_flags && policy_flags.profile_provenance
      && policy_flags.profile_provenance.upi_id && policy_flags.profile_provenance.upi_id.value;
    return isUpiId(v) ? String(v).trim() : null;
  } catch (_) { return null; }
}

/**
 * ⭐ WHICH JURISDICTION IS THIS SHOP IN? The column first, then the rule lib/profile-map.js already declares for
 * this field — "IN when a GSTIN exists" — then the profile. Null when none of them answers, which is a real answer.
 * ⚠️ ISO 3166-1 alpha-2, upper case, because that is what every other layer here already speaks.
 */
function countryOf({ country, gstin, profile }) {
  const c = String(country || '').trim().toUpperCase();
  if (c.length === 2) return c;
  if (gstin && String(gstin).trim()) return 'IN';        /* profile-map: derive 'IN when a GSTIN exists' */
  const pc = profile && String(profile.country || '').trim().toUpperCase();
  return (pc && pc.length === 2) ? pc : null;
}

/**
 * ⭐ HOW CAN THIS SHOP BE PAID? Country decides what is OFFERED; the declared addresses decide what can be DRAWN.
 * ⚠️ Cash is not conditional on anything, and a scheme with no address is not offered — a QR with no payee is worse
 * than no QR.
 */
function payWays({ country, policy_flags }) {
  const cc = String(country || '').toUpperCase();
  const out = [{ id: 'cash', label: 'Cash', qr: null }];
  for (const [id, s] of Object.entries(PAY_SCHEMES)) {
    if (s.countries && cc && s.countries.indexOf(cc) < 0) continue;
    const value = id === 'upi' ? upiOf(policy_flags) : null;
    if (!value) continue;
    out.push({ id, label: s.label, qr: s.encodes, payee: value });
  }
  out.push({ id: 'card', label: 'Card', qr: null });
  return out;
}

module.exports = { isUpiId, upiOf, payWays, countryOf, PAY_SCHEMES, PAY_RECORD_ONLY, profileValues, applyProfileMap, getProfile, saveProfile, getVault, saveVault, resolveProfileReadiness, resolvePath, SECTION_TYPES, VERIFIABLE_TAGS, sanitizeVault, invoiceParty, GST_STATES };
