...
async function getProfile(entity_id) {
  try {
    ...
    if (r.rows[0]) return { trade_mode: r.rows[0].trade_mode, markets: r.rows[0].markets || [], sectors: r.rows[0].sectors || [], adopted: r.rows[0].adopted || [] };
...
module.exports = { getProfile, saveProfile, resolveProfileReadiness, resolvePath };
