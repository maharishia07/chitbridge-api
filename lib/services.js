...
async function shelfOf(entity_id) {
  ...
  for (const it of items) { const d = it.item_data || {}; byId.set(String(it.item_id), d); const c = String(d.code || d.sku || '').trim().toLowerCase(); if (c && !byCode.has(c)) byCode.set(c, d); }
  return { shelf: sh, items, byId, byCode, productOf: (l) => (l.item_id && byId.get(String(l.item_id))) || (l.code && byCode.get(String(l.code).trim().toLowerCase())) || null };
}
...
function rateLines(lines, shelf) {
  const asLines = lines.map((l) => ({ item_id: l.item_id, name: l.name, particulars: l.name, gst_rate: l.gst_rate, cess_rate: l.cess_rate, hsn: l.hsn }));
  const dec = shelf ? T.decorate(asLines, shelf) : asLines;
  return lines.map((l, i) => Object.assign({}, l, { gst_rate: dec[i].gst_rate != null ? num(dec[i].gst_rate) : l.gst_rate, cess_rate: num(dec[i].cess_rate) || l.cess_rate || 0, hsn: dec[i].hsn || l.hsn || null, tax_slab_name: dec[i].tax_slab_name || null, tax_source: dec[i].tax_source || (l.gst_rate != null ? 'caller' : null), tax_scheme: dec[i].tax_scheme || l.tax_scheme || null }));
}
