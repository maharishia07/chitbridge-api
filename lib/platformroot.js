...
async function deskFor(population) {
  ...
  if (deskMemo.byCode && (Date.now() - deskMemo.at) < DESK_TTL_MS) {
    ...
    try {
      ...
      const r = await query('SELECT code, desk_entity_id FROM ops.population');
      byCode = {};
      for (const row of r.rows) byCode[String(row.code).toLowerCase()] = row.desk_entity_id || null;
      deskMemo = { at: Date.now(), byCode };
    } catch (e) {
  ...
  return { entity_id: envAnswer.entity_id || byCode.live || null,
           why: known ? ('no desk named for “' + p + '” — sent to the live desk')
