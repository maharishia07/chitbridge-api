...
const LANGS = [
  ...
  /**
   * ⭐ FOREIGN LANGUAGES — and for this product they are not a nicety (Athi, 2026-08-17: *"any foreign language
   * as well… they can choose to change"*). CB is built around trade LANES, so the language that matters second
   * is the DESTINATION's: an exporter to the Gulf needs Arabic, to Europe needs French or German. The list is
   * ordered by the destinations the trade-lane work already treats as real.
   */
 ...
 */
function normUnit(u, extra) {
  const f = fold(u);
  /**
   * ⚠️⚠️ THE ENTITY'S OWN CONFIRMED SPELLINGS, PASSED IN — never read from a database here. This module is TIER A
   * (zero dependencies) precisely so the folding rule can be reasoned about and tested without a world around
   * it; reaching for storage would end that. `extra` is `{ spelling: canonical }`, already CONFIRMED by a human,
   * supplied by whoever has the entity context.
   *
   * ⚠️ THE CURATED TABLE WINS. An entity may TEACH the system a word it did not know; it may not REDEFINE one it
   * did. Otherwise an accepted AI proposal could quietly re-point `kg` at something else for that entity alone,
   * and every total on the account would shift with nothing to show for it.
   */
  const hit = LOOKUP.get(f);
  if (hit) return hit;
  if (extra) {
    const e = extra[f] || extra[u];
    if (e && LOOKUP.has(fold(e))) return LOOKUP.get(fold(e));   // must resolve to a unit we actually know
  }
  return f;
}
