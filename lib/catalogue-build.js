...
const money = require('./money');     // the one tolerant price reader (a bare number or { amount, currency })
const VERSION = 'v1';

// The brand/store label for a DB-authored source. Titles are published as "Store — Product" (catfPublishBlueprint),
// so the prefix is the store; fall back to the source_key. Replaces the old hardcoded 'Beta' / 'royale-play.pdf'.
function _entityLabel(title, sourceKey) { const t = String(title || '').split(' — ')[0].trim(); return t || sourceKey || 'source'; }
...
async function loadSource(sourceKey, forEntity) {
  ...
  let row = null;
  try {
    /* ⭐ the brand's publish waiting for its moment rides the same read (identities is not row-secured) — see
       lib/network-catalogue.js: applied once, by the first reader after the moment, and served fresh from then */
    const r = await query(
      `SELECT s.source_key, s.version, s.for_vertical, s.title, s.collection, s.schema, s.items, s.commercials_fields,
              s.owner_entity_id, s.experience, s.formatting,
              i.policy_flags #> ARRAY['network_catalogue', 'pending', s.source_key] AS pending,
              CASE WHEN $2 = '' OR s.owner_entity_id IS NULL OR s.owner_entity_id::text = $2 THEN NULL
                   ELSE ${require('./network-membership').isMemberSql('s.owner_entity_id::text', '$2')} END AS is_member
         FROM catalogue_source s LEFT JOIN identities i ON i.identity_id = s.owner_entity_id
        WHERE s.source_key = $1 AND s.active = true LIMIT 1`, [sourceKey, forEntity ? String(forEntity) : '']);
    row = r.rows[0];
    if (row && row.pending) {
      const fresh = await require('./network-catalogue').applyIfDue(row.owner_entity_id, row.pending).catch(() => null);
      if (fresh) row.items = fresh;
    }
  } catch (_) {
    try {
  ...
  if (row && Array.isArray(row.items) && row.items.length) {
    return { _fromDb: true, source: row.source_key, for_entity: _entityLabel(row.title, row.source_key), for_vertical: row.for_vertical,
      ...
      is_member: row.is_member === undefined ? null : row.is_member };
  }
...
async function listSources() {
  try {
    const r = await query(`SELECT source_key, title, items, for_vertical FROM catalogue_source WHERE active = true ORDER BY source_key`);
    if (r.rows.length) return r.rows.map((row) => ({ key: row.source_key, for_entity: _entityLabel(row.title, row.source_key), title: row.title, item_count: (row.items || []).length, for_vertical: row.for_vertical || null, from_db: true }));
  } catch (_) { /* fall through to code */ }
  return Object.keys(SOURCES).map((k) => ({ key: k, for_entity: SOURCES[k].for_entity, title: SOURCES[k].title, item_count: SOURCES[k].items.length, for_vertical: SOURCES[k].for_vertical || null }));
}
...
async function build(sourceKey, forEntity) {
  const s = await loadSource(sourceKey, forEntity); if (!s) return null;
  ...
  return {
    ...
    from_db: !!s._fromDb,
    is_member: s.is_member === undefined ? null : s.is_member,   // the asking store in this brand's network? (network-membership)
...
async function resolve(sourceKey, commercials, opts) {
  const forEntity = (opts && opts.entity_id) || null;
  const b = await build(sourceKey, forEntity); if (!b) return null;
  /**
   * ⭐⭐ ONLY A MEMBER SELLS A BRAND'S PRODUCTS (Athi, 2026-09-17: "the network has to approve"). Asked for a store (opts.entity_id)
   * about a catalogue another brand owns: a store that is not in that brand's network (lib/network-membership — built under it,
   * or joined and approved) gets the catalogue with NO items, and `not_member` says why. Its own prices are untouched, so
   * approval brings everything back as it was. A catalogue nobody owns is open to all, as before.
   */
  if (forEntity && b.is_member === false) { b.items = []; b.not_member = true; b.adopted = Object.keys(commercials || {}).length; return b; }
  const com = commercials || {};
  /**
   * ⭐ TWO RULES FROM THE BRAND'S PUBLISH (lib/network-catalogue.js):
   *   a RENAMED product keeps the store's overlay — the old name is looked up through `formerly`;
   *   a store that FOLLOWS the brand on a product is priced at the brand's current suggestion (its price, else its MRP),
   *   so the store moves when the brand publishes, and a store that set its own price never does.
   */
  const suggest = (it) => { const p = money.amountOfLoose(it.price); if (it.price != null && it.price !== '' && Number.isFinite(p)) return p;
                            const m = money.amountOfLoose(it.mrp); return (it.mrp != null && it.mrp !== '' && Number.isFinite(m)) ? m : null; };
  b.items = b.items.map((it) => {
    let c = com[it.name] || (Array.isArray(it.formerly) ? it.formerly.map((f) => com[f]).find(Boolean) : null) || null;
    if (c && c.follow) {
      const s = suggest(it);
      if (s !== null) {
        const cur = (c.price && typeof c.price === 'object' && c.price.currency) || null;
        c = Object.assign({}, c, { price: cur ? { amount: s, currency: cur } : s });
      }
    }
    return Object.assign({}, it, { commercials: c });
  })
  /* ⭐ the brand withdrew it from THIS store (lib/network-catalogue — per-store withdrawal): not offered here, anywhere */
    .filter((it) => !(it.commercials && it.commercials.brand_withdrawn));
