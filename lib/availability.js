...
/**
 * countedZero(item_data) — has somebody actually counted, and counted none?
 *
 * ⚠️⚠️ ABSENT IS NOT ZERO, and that distinction is the whole reason this is a function rather than `!qty`.
 * `avail` is a FEED somebody else keeps, and most shops keep none — so "nobody said" and "the shelf is empty"
 * are different claims, and only the second is a fact. `!qty` conflates them and would answer true for every
 * product in every shop that has never reported stock.
 *
 * ⭐ The same line the catalogue screen draws when it prints `0` for a counted empty shelf and `N/A` for an
 * unmeasured one. One rule, both ends.
 *
 * Only `policy.qty_zero_hides = 'on'` acts on this, and it HIDES rather than stamping a status — see
 * lib/policy.js. Athi, 2026-09-01: *"ours is not an inventory system, so we don't handle the quantity, so
 * explicit marker is best."*
 */
function countedZero(item_data) {
  const a = item_data && item_data.avail;
  if (!a || typeof a !== 'object') return false;         // no feed at all — unknown, not empty
...
module.exports = { stamp, countedZero, freshness, distanceKm, eta, answer, SOURCES, FRESH };
