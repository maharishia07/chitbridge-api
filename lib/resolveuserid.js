...
function classify(typed) {
  ...
  if (handle.isEmployee(h)) {
    ...
    /**
     * ⭐ `root` AS WELL AS `at`. Athi, 2026-09-16, after b260 ran: *"just check the root is also resolved,
     * because in network we may need to provide access to all the entity for the network users."*
     * `at` is the STORE she works at (`acmetraders.clothing`); `root` is the NETWORK it was born in
     * (`acmetraders`). For a plain business the two are the same string. Nothing is decided here about what a
     * root grants — that is his call when it is tested — this only makes the fact available to whoever decides.
     */
    return p ? { kind: 'employee', actor_key: p.actor_key, at: p.at, root: handle.rootOf(p.at), handle: h }
  ...
  if (at > 0) {
    ...
    return {
      kind: 'employee_typed', actor_key: left, at: right, root: handle.rootOf(right),
