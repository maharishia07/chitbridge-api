...
function ensureTable() {
  ...
  _ready = (async () => {
    // Under the least-privilege RLS role (cb_app) DDL is NOT permitted (no CREATE on schema public), so NEVER
    // attempt CREATE TABLE when the table already exists — that itself raises "permission denied for schema public"
    // and kills the write. Check first; the table is provisioned by migration b65. Only a fresh/admin DB creates it.
    const chk = await query(`SELECT to_regclass('public.cb_attachment') AS t`);
    if (chk.rows[0] && chk.rows[0].t) return;   // exists → nothing to do (the common path)
    try {
    ...
    } catch (e) { console.warn('[storage] cb_attachment not present and auto-create failed (run migration b65 as admin):', e.message); }
  })();
  return _ready;
}
