...
async function brandView(brand_id) {
  ...
  /**
   * ⭐ WHO IS IN THE NETWORK — named, not only counted (Athi, 2026-09-17: "we should know they are part of the network").
   * A store joins by adopting the brand's catalogue; a store may adopt several brands and so sit in several networks —
   * each brand sees only its own members, and each store sees each of its brands (the store half of GET /network-offers).
   */
  const memberMap = flags.network_members || {};
  const ids = Object.keys(memberMap);
  const members = (await memberNames(ids)).map((m) => Object.assign(m, { joined_at: (memberMap[m.id] || {}).at || null }));
  return {
    members,
 ...
 *   opts.noteMembers  backfill the brand's member index (default true) — the counter does; a public storefront view does not
...
async function forStore(store_id, owners, linesByOwner, opts) {
  ...
  const brands = [...new Set((owners || []).filter(Boolean).map(String))].filter((b) => b !== String(store_id));
  const perBrand = await Promise.all(brands.map(async (brand_id) => {
    if (o.noteMembers !== false) noteMember(brand_id, store_id);   /* backfills stores that adopted before the index existed */
...
async function forFinishes(store_id, finishes, opts) {
  ...
  return forStore(store_id, owners, byOwner, Object.assign({ noteMembers: false }, opts || {}));
}

/**
 * ⭐ WHETHER A BRAND'S STORES HAVE EACH OFFER — stamped onto definition rows (routes/definitions list).
 *   net = { released: policy_flags.network_offers.released, stores: count of network_members }
...
module.exports = { memberNames, stampReleases, POLICIES, brandName, nextOpening, whenOf, mergeFlag, setIn, setReleased, flagsOf, noteMember, pushToNetwork, brandView, forStore, forFinishes, lineKey };
