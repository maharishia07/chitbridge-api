...
async function brandView(brand_id) {
  ...
  const nm = ids.length ? await query('SELECT identity_id, display_name FROM identities WHERE identity_id = ANY($1::uuid[])', [ids])
    .catch(() => ({ rows: [] })) : { rows: [] };
  const members = nm.rows.map((r) => ({ id: String(r.identity_id), name: r.display_name || null,
                                        joined_at: (memberMap[String(r.identity_id)] || {}).at || null }))
...
module.exports = { stampReleases, POLICIES, brandName, nextOpening, whenOf, mergeFlag, setIn, setReleased, flagsOf, noteMember, pushToNetwork, brandView, forStore, forFinishes, lineKey };
