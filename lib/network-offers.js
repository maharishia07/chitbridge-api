...
async function brandView(brand_id) {
  ...
  return {
    ...
    offers: r.rows.map((d) => ({ id: d.definition_id, name: d.name, kind: d.sub_kind, status: d.status,
      released: rel[d.definition_id] || null })),
  };
}
...
async function forStore(store_id, owners, linesByOwner) {
  ...
  for (const brand_id of [...new Set((owners || []).filter(Boolean).map(String))]) {
    ...
    const live = await catalogueView.liveOffers({ entity_id: brand_id, withEntity, viewer: null, groups: [], all: true });
    const want = new Set(ids);
    const offers = (Array.isArray(live) ? live : []).filter((o) => want.has(String(o.id)));
    ...
    for (const o of offers) {
      ...
      copy.network = { brand_id, brand_name: shownAs, policy, released_at: r.released_at || null,
