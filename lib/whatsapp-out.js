...
async function notify(entity_id, { to_ref, from_ref, provider_ref, text, chit_id, capture_id }) {
  ...
  if (!win.open) return _log(entity_id, { chit_id, capture_id, from_ref, to_ref, body: text, status: 'refused', reason: win.reason });

  try {
    const res = await fetch(GRAPH + '/' + encodeURIComponent(provider_ref) + '/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + process.env.WHATSAPP_TOKEN },
      body: JSON.stringify({ messaging_product: 'whatsapp', to: String(to_ref).replace(/^\+/, ''), type: 'text',
