...
/**
 * ⚠️⚠️⚠️ [OFFR-04] THERE IS NO canSeeCosts() HERE — ON PURPOSE, AFTER GETTING IT WRONG ONCE. A version of
 * this function briefly lived here, reading a NEW policy_flags.offer_lab_costs_visible_to_staff — one
 * entity-wide switch invented for Offer Lab specifically. lib/cost.js (b145) already answers the identical
 * question — "may this identity see cost/margin" — the RIGHT way: a per-actor identities.can_see_costs
 * column, granted one co-assist at a time through the existing PATCH /api/actors/:id (owner-only, guarded
 * there), with its own audit trail and its own "Sees costs" chip already live in app/cap-admin.js. Two
 * mechanisms answering the same question is exactly what this file's own header warns about — "eighteen
 * places... any new rule had to be remembered in all of them." Reuse lib/cost.js's canRead(req, entity_id)
 * for this; do not rebuild it here or anywhere else. [[feedback-adopt-dont-reinvent]] [[feedback-no-duplicate-functions]]
 */
