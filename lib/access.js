...
/**
 * May this identity see an item's cost/margin? ([OFFR-04], 2026-08-20's own PRESETS already named this —
 * `can_see_costs` sat on every preset above as description text for the picker, with nothing reading it
 * back. Wiring it up, not inventing it.)
 *
 * Athi, 2026-09-25: *"the cost information... should not be visible for employee ie the cost information
 * unless the access is provided"* — then, asked to decide the boundary: *"employees should not see
 * without a specific permission. otherwise, leave it with owner only."*
 *
 * ⚠️ THE ENTITY ALWAYS SEES ITS OWN COSTS — same reasoning as levelOf(): it owns the records, there is no
 * permission above it to grant one, and there is nobody else who could turn this on for an owner locked out
 * of their own numbers.
 * ⚠️ AN ACTOR SEES COSTS ONLY IF THE ENTITY EXPLICITLY SAID SO — entityFlags is the OWNER's own
 * policy_flags (never the actor's own row; an actor cannot grant this to themselves), and the default
 * — the flag absent, or the caller not even passing entityFlags — is FALSE. Unlike levelOf()'s "absent
 * means editor" (a hardening must not silently demote existing staff), cost visibility is new: there is no
 * prior behaviour to preserve, so the safe default is the restrictive one.
 */
function canSeeCosts(identity, entityFlags) {
  if (!identity) return false;
  if (identity.identity_type !== 'actor') return true;
  return !!(entityFlags && entityFlags.offer_lab_costs_visible_to_staff);
}
...
module.exports = {
  ...
  levelOf, canEdit, canMessage, canRaiseDispute, seesWholeEntity, canSeeCosts,
};
