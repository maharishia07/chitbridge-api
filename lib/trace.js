...
function buildEdge(input) {
  ...
  // Optional network scope: names the bounded network + its OPERATOR (an entity that is mandated to co-hold this
  // edge for oversight). The operator's co-hold is what lets it walk its whole subtree — visibility ≡ co-holding.
  let network = null;
  if (input.network && typeof input.network === 'object' && !Array.isArray(input.network)) {
    const nid = (typeof input.network.id === 'string' && input.network.id.trim()) ? input.network.id.trim().slice(0, 64) : null;
    const op  = (typeof input.network.operator === 'string' && UUID_RE.test(input.network.operator)) ? input.network.operator : null;
    if (nid || op) network = { id: nid, operator: op };
  }
  return { edge: { parents, is_origin: isOrigin, product, qty, unit, base_qty, base_unit, unit_known, ...(network ? { network } : {}) } };
}
