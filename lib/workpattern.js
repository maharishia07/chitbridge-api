...
async function entityConstitution(entity_id) {
  ...
  try {
    ...
    const row = r.rows[0];
    if (row && row.key) return versionref.format(row.key, row.ver);
...
async function resolveConstitutionRef(entity_id) {
  ...
  try {
    ...
    const row = r.rows[0]; if (row && row.key) return versionref.format(row.key, row.version);
  } catch (_) {
    try { const r2 = await query(`SELECT constitution_key AS key, version FROM constitution WHERE active = true ORDER BY minted_at DESC LIMIT 1`);
      const row2 = r2.rows[0]; if (row2 && row2.key) return versionref.format(row2.key, row2.version); } catch (_2) {}
  }
  return null;
}
