...
async function resolveWorkPattern(id, ctx) {
  ...
  if (!refs) {
    ...
    refs = {
      ...
      _standard: await resolveStandardRef(ctx),                     // canonical SHARED source (ISO 9000...) assimilated into the boilerplate
    };
    _memoSet(key, refs);
  }
  const cfg = Object.assign({}, refs.sealed, refs.open);
  cfg._blueprint = refs._blueprint; cfg._capability = refs._capability;
  cfg._constitution = refs._constitution; cfg._standard = refs._standard;
  if (id === 'iot-signal') {
