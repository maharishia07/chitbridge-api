const versionref = require('./versionref');
...
// LOOSE COUPLING (2026-07-11): the only code-baked standard is this LAST-RESORT self-heal seed, used solely when
// standard_source is empty/absent (fresh/broken DB). WHICH standards apply is DATA — declared by the boilerplate
// (b88) or read live from standard_source — never a hardcoded preference in code. So renaming/swapping a standard is
// a data change (one row), not a code change. See [[feedback-loose-until-stamped]]. `facet` here = the CATEGORY
// ('quality'), which is stabler than any specific standard key, and only picks the back-compat singular `_standard`.
const STANDARD_SEED = { key: 'iso-9001', version: 'v1', facet: 'quality' };

// resolveStandards(ctx) → the FULL set of canonical standards, ONE per facet → { <facet>: 'key@version' }. Every active
// facet is read GENUINELY from standard_source (b85), latest version per facet — NO hardcoded preference. Self-healing:
// table absent/empty → the last-resort seed. (This is the GLOBAL fallback; a boilerplate's DECLARED standards win when
// the entity has one — resolveWorkPattern picks the BP first, this only backstops the pre-boilerplate path.)
async function resolveStandards(_ctx) {
  ...
  try {
    ...
    for (const row of r.rows) if (row.facet && row.key) map[row.facet] = versionref.format(row.key, row.version);
  } catch (_) { /* standard_source not present yet → seed below */ }
  if (!Object.keys(map).length) map[STANDARD_SEED.facet] = versionref.format(STANDARD_SEED.key, STANDARD_SEED.version);
  return map;
}
...
async function mintedCapabilityFor(patternId) {
  try {
    ...
    const row = r.rows[0];
    if (row && row.key) return versionref.format(row.key, row.cver);
  } catch (_) { /* capability catalogue / parent column not present yet */ }
  return null;
}

/**
 * ── ⭐⭐ THE GOVERNING CONSTITUTION FOR THIS RUN — asked of lib/govresolve, not re-derived here ────────────────
 *
 * Athi, 2026-09-16: *"i hope you are keeping it as a single source and nothing to be found from sweeping the
 * entire code base?"* Sweeping found THIS. It read the entity's stamp itself and, for an unstamped entity, fell
 * back to the `is_default` constitution — while lib/govresolve fell back to `base`. Two resolvers, two different
 * answers to "who governs this entity", and nothing anywhere said so.
 *
 * ⚠️ THE FALLBACK CHANGES, AND IT IS SAID OUT LOUD: an unstamped entity's work pattern is now governed by `base`
 * (the universe), not by whichever constitution carries `is_default`. Only legacy rows are unstamped — b254's
 * trigger and registration both stamp — so this is a correction of the rare path, not the common one. One
 * cascade, one default; a second default is exactly the drift being removed. [[feedback-no-duplicate-functions]]
 */
async function resolveConstitutionRef(entity_id) {
  if (!entity_id) return null;
  try {
    const g = await require('./govresolve').resolveEntityGovernance(entity_id);
    return (g && g.constitution) || null;
  } catch (_) { return null; }   /* best-effort, as before: a work pattern must never fail on a governance read */
}
...
async function resolveWorkPattern(id, ctx) {
  ...
  if (!refs) {
    ...
    refs = {
      ...
      _blueprint: versionref.format(id, minted ? minted.version : 'code'),  // the version-frozen reference for the audit stamp
      ...
      _boilerplate: bpl ? versionref.format(bpl.key, bpl.version) : null,   // the mould this entity was minted from (or null)
    };
  ...
  if (id === 'iot-signal') {
    ...
    /**
     * ── ⭐ ONE MORE RUNG: device → connector → KIND → entity default (b250) ────────────────────────────────
     *
     * DESIGN-SUPPORT-LIFECYCLE.md §5.1. `entity_actor_settings.default_assignee_actor_id` is ONE assignee for
     * the whole business, so "incidents to support, requirements to product" could not be said at all.
     *
     * ⚠️ IT SITS BETWEEN connector AND entity, which is exactly where "most specific wins" puts it — so a kind
     * with no rule resolves as it always did, and no routing that works today can change.
     */
    let da = dev.default_assignee || con.default_assignee || null;
    let fol = null;
    if (ctx.entity_id && ctx.kind) {
      const r = await require('./workroute').routeFor(ctx.entity_id, ctx.kind);
      if (!da) da = r.assignee_actor_id || null;
      fol = r.folder_id || null;
      cfg._routing = r.why;                 /* ⭐ said, so an unrouted queue can be SEEN to be unrouted */
    }
    if (!da && ctx.entity_id) {
      try {
    ...
    if (fol) cfg.folder = fol;              /* ⚠️ only when set: never overwrite the device/connector folder */
  }
  return cfg;
}
