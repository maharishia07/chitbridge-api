...
// The active CONSTITUTION → `constitution@version` (the top rung the entity adopts). Best-effort / self-healing.
async function activeConstitution() {
  try {
    const r = await query(`SELECT constitution_key AS key, version FROM constitution WHERE active = true ORDER BY minted_at DESC LIMIT 1`);
    const row = r.rows[0];
    if (row && row.key) return row.key + '@' + (row.version || 'v1');
  } catch (_) { /* constitution catalogue not present yet */ }
  return null;
}
...
async function resolveWorkPattern(id, ctx) {
  ...
  cfg._constitution = await activeConstitution();                  // the governing constitution@version (top rung)
  ctx = ctx || {};
  if (id === 'iot-signal') {
