      ...
      if (ids.length) {
        ...
        try {
          locked = await db.query(
            `SELECT DISTINCT d.line_id, l.particulars
               FROM chit_line_delivery d
               LEFT JOIN chit_line l
                      ON l.entity_id = d.entity_id AND l.chit_id = d.chit_id AND l.line_id = d.line_id
              WHERE d.entity_id = $1 AND d.chit_id = $2 AND d.line_id = ANY($3::uuid[])`,
            [entity_id, chit_id, ids]);
        } catch (e) {
          /* Pre-b144 there are no deliveries, so there is nothing to lock. An un-migrated environment must not
             start refusing corrections it has always accepted. */
          if (!notMigrated(e)) throw e;
        }
        if (locked.rows.length) {
          const names = locked.rows.map((r) => r.particulars).filter(Boolean);
          const e = new Error(
            (names.length === 1 ? '“' + names[0] + '” has' : locked.rows.length + ' of these lines have')
            + ' already been delivered, so the order cannot be changed for '
            + (names.length === 1 ? 'it' : 'them') + '. If the delivered figure is wrong, record a correcting '
            + 'delivery instead — that keeps both the original and the correction on the record.');
          e.status = 409; e.code = 'LINE_DELIVERED';
          throw e;
        }
      }
