    ...
    return await withEntity(entity_id, async (db) => {
      ...
      if (ids.length) {
        ...
        for (const p of prepared) {
          ...
          if (p.line.unit !== undefined && uKey(p.line.unit) !== uKey(cur.ordered_unit)) {
            const e = new Error('The unit of ' + name + ' cannot be changed from ' + (cur.ordered_unit || '—')
              + ' to ' + p.line.unit + ' — ' + outQty + ' has already been delivered in the original unit, and '
              + 'changing it would leave that delivery counting against nothing.');
