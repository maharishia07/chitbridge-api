  ...
  try {
    return await withEntity(entity_id, async (db) => {
      ...
      if (ids.length) {
        ...
        try {
          /**
           * ⚠️ ONE DEFINITION OF "DELIVERED", NOT A COPY OF IT.
           *
           * This began as an inline sum here — signed, `deliver` only, in the line's own unit. Then the worklist
           * needed the same figure, and two hand-written copies of one rule is how two screens start disagreeing
           * about how much is owed. b153's `chit_line_delivered_qty()` is now the single definition and both
           * callers ask it. It also fixes what the inline copy got right by accident and b152's function got
           * wrong: only MY OWN claim counts, never the counterparty's.
           */
          live = await db.query(
