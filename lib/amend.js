...
async function record(entity_id, chit_id, edits, who = {}) {
  ...
  try {
    return await withEntity(entity_id, async (db) => {
       ...
       * ── ⭐ THE RECEIPT LOCK ───────────────────────────────────────────────────────────────────────────────────
       *
       * Athi, 2026-08-14: *"once the receipt is made, then no more amendments. So in the order page should be
       * greyed. We can think about it, how to protect."*
       *
       * ⚠️ THE PROTECTION LIVES HERE, NOT IN THE GREYING. A disabled button hides the action from one screen while
       * the API keeps accepting it — and everything that is not that screen (a connector, a retry, a second tab,
       * the next UI someone writes) walks straight through. The UI greying is the courtesy; this is the rule.
       *
       * ⚠️ PER LINE, NOT PER CHIT — a deliberate choice. Freezing all thirteen lines because two shipped would
       * block correcting the eleven that have not, and a chit is line-level work. So a line with ANY delivery
       * against it is settled; its neighbours stay amendable.
       *
       * ⚠️ AND IT REFUSES A REMOVAL TOO. You cannot un-order something that has already been handed over. Where
       * the delivered figure itself is wrong, b144 takes a NEGATIVE correcting entry — the record gains a
       * correction rather than losing the original, which is the same discipline as an amendment.
       *
       * ONE statement for every edit in the batch: at ~250ms per statement on the cloud link, a per-line check
       * would cost more than the write it guards.
       */
      const ids = prepared.map((p) => p.line_id).filter(Boolean);
      if (ids.length) {
        let live = { rows: [] };
        try {
          /* ⚠️ SIGNED SUM, `deliver` ONLY, IN THE LINE'S OWN UNIT — the same three rules lib/deliverline.js
             applies. A negative correcting entry must reduce what is considered delivered (otherwise a wrongly
             recorded 58 would block the order forever), an 'add' never draws down a quantity, and 12 pieces are
             not 12 kg. A guard that ignored any of the three would refuse a legitimate amendment using a number
             that was never true. */
          live = await db.query(
            `SELECT l.line_id, l.particulars, l.unit AS ordered_unit,
                    COALESCE(SUM(d.quantity) FILTER (
                      WHERE COALESCE(to_jsonb(d)->>'kind','deliver') = 'deliver'
                        AND COALESCE(NULLIF(btrim(lower(d.unit)),''), btrim(lower(COALESCE(l.unit,''))))
                          = btrim(lower(COALESCE(l.unit,'')))), 0) AS delivered
               FROM chit_line l
               LEFT JOIN chit_line_delivery d
                      ON d.entity_id = l.entity_id AND d.chit_id = l.chit_id AND d.line_id = l.line_id
              WHERE l.entity_id = $1 AND l.chit_id = $2 AND l.line_id = ANY($3::uuid[])
              GROUP BY l.line_id, l.particulars, l.unit`,
            [entity_id, chit_id, ids]);
        } catch (e) {
          /* Pre-b144 there are no deliveries, so there is nothing to guard. An un-migrated environment must not
             start refusing corrections it has always accepted. */
          if (!notMigrated(e)) throw e;
        }

        /**
         * ── ⭐ PARTIAL DELIVERY STAYS AMENDABLE ──────────────────────────────────────────────────────────────
         *
         * Athi, 2026-08-14: *"partial delivery can be amendable, that is what makes it interesting."* Author's
         * decision, and it replaces the rule he set on 2026-08-13 (*"once the receipt is made, then no more
         * amendments"*), which I implemented as "any delivery at all locks the line".
         *
         * ⚠️ THIS IS A RELAXATION OF A SHIPPED GUARD, MADE ON THE AUTHOR'S EXPLICIT INSTRUCTION, AND IT IS
         * FLAGGED RATHER THAN DONE QUIETLY. The old rule was too early: 40 of 100 delivered froze the 60 still
         * owed, which is precisely the part someone legitimately needs to change. The receipt — settlement — is
         * the right lock, and "a delivery happened" was only ever a proxy for it.
         *
         * ⚠️ SO IT IS REPLACED BY THREE NARROWER RULES, NOT BY NOTHING. Net, these refuse things the old blanket
         * lock also refused, and permit only the case he named:
         *
         *   1. QUANTITY MAY NOT FALL BELOW WHAT HAS GONE OUT. Amending 100 down to 20 when 40 was delivered is
         *      not an amendment, it is a return — it needs a negative event so both the original and the
         *      correction stay on the record. Without this rule, "amendable" is a quiet way to make delivered
         *      goods disappear from the arithmetic, which is the worst thing an order document can do.
         *   2. THE UNIT MAY NOT CHANGE ONCE ANYTHING IS DELIVERED. This one is created by the mixed-unit fix
         *      itself: change a line from kg to piece and every existing kg delivery becomes off-unit, so
         *      `delivered` silently drops to zero and the line reopens fully. A hazard that did not exist this
         *      morning, closed the same day it was introduced.
         *   3. A DELIVERED LINE MAY NOT BE REMOVED. Unchanged from the old rule. You cannot un-order something
         *      that has already been handed over.
         */
        const byId = new Map(live.rows.map((r) => [r.line_id, r]));
        const uKey = (u) => String(u == null ? '' : u).trim().toLowerCase();
        for (const p of prepared) {
          const cur = p.line_id ? byId.get(p.line_id) : null;
          if (!cur) continue;
          const delivered = Number(cur.delivered) || 0;
          if (delivered <= 0) continue;                       // nothing has gone out — amend freely
          const name = cur.particulars ? '“' + cur.particulars + '”' : 'this line';
          const outQty = delivered + (cur.ordered_unit ? ' ' + cur.ordered_unit : '');

          if (p.line === null) {
            const e = new Error(name + ' cannot be removed — ' + outQty + ' has already been delivered against it. '
              + 'Record a negative delivery instead, which keeps both the original and the correction on the record.');
            e.status = 409; e.code = 'LINE_DELIVERED'; throw e;
          }
          /**
           * ⚠️ AN OMITTED UNIT IS A CHANGE TO NULL, NOT "LEAVE IT ALONE" — and reading it as the latter was a
           * live bypass of this whole guard, found by the e2e suite an hour after the guard was written.
           *
           * An amendment stores a full REPLACEMENT line, so a payload without `unit` writes NULL over "kg". Every
           * existing kg delivery then belongs to a different unit than the line, `delivered` computes as 0, and
           * the below-delivered rule below waves through a reduction it exists to refuse. The screen never does
           * this — amendSave copies the whole live line — but the API accepted it from anything else, which is
           * precisely the class of hole that only shows up when something other than the screen calls in.
           */
          const newUnit = p.line.unit === undefined ? '' : p.line.unit;
          if (uKey(newUnit) !== uKey(cur.ordered_unit)) {
            const e = new Error('The unit of ' + name + ' cannot be changed from ' + (cur.ordered_unit || '—')
              + ' to ' + (String(newUnit).trim() || 'nothing') + ' — ' + outQty + ' has already been delivered in '
              + 'the original unit, and changing it would leave that delivery counting against nothing. Send the '
              + 'unit unchanged with the correction.');
            e.status = 409; e.code = 'LINE_UNIT_LOCKED'; throw e;
          }
          if (p.line.quantity !== undefined && Number(p.line.quantity) < delivered) {
            const e = new Error(name + ' cannot be reduced to ' + p.line.quantity + ' — ' + outQty
              + ' has already been delivered. To send some back, record a negative delivery; that keeps both the '
              + 'original and the return on the record.');
            e.status = 409; e.code = 'LINE_BELOW_DELIVERED'; throw e;
          }
        }
      }
