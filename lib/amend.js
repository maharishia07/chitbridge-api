...
const REASONS = ['misread_by_ai', 'customer_clarified', 'rate_agreed', 'stock_unavailable', 'other'];
...
async function record(entity_id, chit_id, edits, who = {}) {
  ...
  const prepared = rows.map((r) => {
    ...
    return { idx, line: clean(r.line === null ? null : r.line),
    ...
    return await withEntity(entity_id, async (db) => {
      ...
      for (const p of prepared) {
        const q = await db.query(
          `INSERT INTO chit_line_amendment (chit_id, entity_id, line_index, seq, line, reason_code, reason, actor_id, actor_name)
           SELECT $1,$2,$3,
                  COALESCE((SELECT MAX(seq) FROM chit_line_amendment
                             WHERE entity_id=$2 AND chit_id=$1 AND line_index=$3), 0) + 1,
                  $4::jsonb,$5,$6,$7,$8
           RETURNING amendment_id, line_index, seq, line, reason_code, reason, actor_name, created_at`,
          [chit_id, entity_id, p.idx, p.line === null ? null : JSON.stringify(p.line),
...
async function list(entity_id, chit_id) {
  try {
    const r = await withEntity(entity_id, (db) => db.query(
      `SELECT amendment_id, line_index, seq, line, reason_code, reason, actor_name, created_at
...
module.exports = { record, list, listFor, liveSet, liveLines, clean, LINE_FIELDS, REASONS };
