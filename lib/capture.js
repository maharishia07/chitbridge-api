...
async function createCapture(entity_id, { channel, sender_ref, sender_name, subject, raw_text, media_refs }) {
  ...
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      `INSERT INTO capture (entity_id, channel, sender_ref, sender_name, subject, raw_text, media_refs)
       VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb) RETURNING id, channel, sender_ref, sender_name, subject, raw_text, status, created_at`,
      [entity_id, ch, sender_ref || null, sender_name || null, subject || null, text.slice(0, 8000),
       JSON.stringify(Array.isArray(media_refs) ? media_refs.slice(0, 20) : [])]));
    return r.rows[0];
  } catch (e) { throw _missing(e); }
}

async function listPending(entity_id) {
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      `SELECT id, channel, sender_ref, sender_name, subject, raw_text, media_refs, status, structured, chit_id, created_at
...
async function structureCapture(entity_id, id) {
  ...
  const ai = require('./ai');
  const out = await ai.invokeSkill(entity_id, 'message-to-chit', {
