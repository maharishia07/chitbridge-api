...
async function raisePayload(entity_id, id) {
   ...
   *
   * ⚠️ CREATED PROVISIONAL, AND OFF THE STOREFRONT. `is_active` is what the public shop reads, so an unpriced row
   * created from a stranger's message would otherwise appear on your shop front at no price. It is stamped
   * `provisional:true` and catalogue-view excludes those — visible to you, invisible to buyers, until you price it.
   *
