...
async function raisePayload(entity_id, id, opts) {
  /**
   * ⚠️ `useCatalogue` — Athi, 2026-08-09: *"if the catalogue data is there it has to consider it? or we can set it
   * up as a flag to consider the catalogue or not."*
   *
   * A flag, and it earns its place — because A CATALOGUE PRICE IS A SELL-SIDE PRICE. For a shop taking an order
   * that is exactly right. For a factory receiving milk from a farmer it is the wrong side of the trade entirely:
   * pricing an inbound supply notice off what the factory SELLS at would put a number on the record that nobody
   * agreed and that flatters one party. Same pipeline, opposite meaning, and only the business knows which it is.
   *
   * Default ON: with it on, an unmatched line is untouched, so the only effect is prices appearing where they
   * genuinely match. Turning it off makes every line carry exactly what the message said and nothing more.
   */
  const useCatalogue = !(opts && opts.useCatalogue === false);
  const cap = await getCapture(entity_id, id);
  if (!cap) { const e = new Error('Capture not found'); e.status = 404; throw e; }
