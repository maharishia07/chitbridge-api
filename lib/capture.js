...
async function raisePayload(entity_id, id) {
  ...
  /**
   * ⚠️ AN ITEM YOU DO NOT STOCK IS STILL REAL DEMAND. Athi, 2026-08-09: *"if the item not found, create an entry
   * and with no price."* Right: a customer asking for something twice is the catalogue telling you what to add,
   * and the alternative is that the request quietly names a thing your system has never heard of.
   *
   * ⚠️ IT DOES NOT REACH YOUR SHOP FRONT, AND NOT BECAUSE OF ANYTHING HERE. The storefront already refuses to list
   * an unpriced line on a monetary shop (catalogue-view's `unpriced_hidden`, which exists because adopting six
   * finishes and pricing two once listed all six). So a row created with no price is hidden by the rule that was
   * already there, and it appears the moment you price it.
   *
   * I first added a second filter that hid anything stamped `provisional`. That reinvented an existing rule AND
   * broke it: nothing clears the flag, so a provisional item you later priced would have stayed invisible forever.
   * The flag is kept as a MARKER — it says where the row came from — and governs nothing.
   *
   * ⚠️ AND IT IS HUMAN-GATED BY CONSTRUCTION. Nothing here runs from the webhook: a person pressed Raise. A public
   * endpoint that could write rows into your catalogue would be a stranger editing your shop.
   */
  const created = [];
  async function ensureItem(name, unit) {
    const n = norm(name); if (!n || byName[n]) return;
    try {
      const ins = await withEntity(entity_id, (c) => c.query(
        `INSERT INTO catalogue_items (entity_id, item_data) VALUES ($1, $2::jsonb) RETURNING item_id`,
        [entity_id, JSON.stringify({
          name: String(name).slice(0, 200), unit: unit || 'unit',
          provisional: true,                                  // ⚠️ keeps it off the storefront until priced
          source: String(cap.channel || 'channel') + ' request',
          first_asked_at: cap.created_at, first_asked_by: cap.sender_ref || null,
        })]));
      if (ins.rows[0]) { byName[n] = { name, unit }; created.push(name); }
    } catch (_) { /* a catalogue that refuses the row must not lose the request */ }
  }
  ...
  const line_items = (s.line_items || []).slice(0, 50).map((l) => {
    ...
    if (m.hit) {
      ...
      /* ⚠️ NOT CREATED, and not priced either. Several catalogue lines already answer to this name, so a new row
         would be a duplicate and a price would be a coin toss. A person picks. */
      priced.ambiguous.push({ name, matches: m.n });
    } else if (m.why === 'not stocked') {
      wanted.push({ name, unit: l.unit || null });     // created after the map — an INSERT cannot ride inside it
    }
    if (!price) priced.unpriced++;
    return { particulars: name, quantity: qty, price, total: Math.round((qty * price + Number.EPSILON) * 100) / 100, ...(unit ? { unit } : {}) };
  });
  for (const w of wanted) await ensureItem(w.name, w.unit);      // demand becomes a catalogue row, unpriced
    ...
    business_json: {
      via: {
        ...
        priced: {
          ...
          created: created.slice(0, 5), created_count: created.length,
