...
async function raisePayload(entity_id, id, opts) {
   ...
   *
   * ⚠️ THIS IS THE WEAK VERSION, DELIBERATELY, AND IT FLAGS RATHER THAN REJECTS.
   * The strong check compares a line's quantity against ITS OWN `raw_phrase`, and rejects on disagreement — the
   * handover doc's T-1. We cannot do that yet: the skill does not emit `raw_phrase`, so there is nothing per-line
   * to compare against (b141 adds it). What we CAN do safely is ask whether the extracted number appears ANYWHERE
   * in the message's numerals. A number that appears nowhere in what the customer wrote was invented.
   *
   * That check cannot produce a false rejection — every legitimate quantity is a number they typed — but it is
   * partial: with several lines, a wrong quantity that coincides with another line's number slips through. So it
   * FLAGS and keeps the value. Nulling a quantity on a partial check would destroy good orders to catch some bad
   * ones. When b141 lands, this becomes per-line and starts rejecting.
   */
  const numerals = require('./numerals');
  const msgNums = numerals.numeralsIn(cap.raw_text || '').map((n) => n.value);

  const priced = { from_catalogue: 0, unpriced: 0, ambiguous: [], asked_differs: [], variant_unspecified: [], quantity_unverified: [] };
  let currency = null;
  const line_items = (s.line_items || []).slice(0, 50).map((l) => {
    ...
    /* ⚠️ ONLY WHEN THE MODEL ACTUALLY EXTRACTED ONE. `l.qty == null` means we defaulted to 1; checking a default
       against the message would flag every unquantified line as invented. */
    const qtyUnverified = (l.qty != null) && msgNums.length > 0 && !msgNums.includes(qty);
