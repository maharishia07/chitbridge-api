...
async function resolveMany(ids) {
  ...
  try {
    ...
    const pairs = c.rows.map((r) => versionref.format(r.container_id, r.current_version));
    const v = await query(
...
function itemContainerId(sourceKey, name) { return versionref.keyOf(sourceKey) + '#' + _norm(name); }
