...
async function ensureDefaultSchema(entity_id) {
  ...
  try {
     ...
     *
     * ⚠️ `quantity` IS THE OPPOSITE PROBLEM AND IS LEFT ALONE DELIBERATELY. csv-preflight.BLOCKED refuses it
     * outright — *"quantity is what a customer orders, not something a product carries"* — and validateAgainst
     * skips it, so it is a declared column the rest of the system treats as not-a-column. Removing it would
     * change what every existing default schema declares, which is a decision, not a cleanup. Flagged for Athi;
     * see catalogue/QUESTIONS.md.
     */
    await query(
      `INSERT INTO schema_fields (schema_id, field_name, field_key, field_type, required, min_value, display_order) VALUES
         ...
         ($1, 'Quantity', 'quantity', 'number', true, 1,    4)`, [schema_id]);
