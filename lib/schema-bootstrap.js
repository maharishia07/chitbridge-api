...
async function ensureDefaultSchema(entity_id) {
  ...
  try {
    ...
    /**
     * ⚠️⚠️ `unit` WAS MISSING, AND THE LIVE PARITY CHECK FOUND IT ON DAY ONE. lib/column-rules.LOCKED names
     * **name · unit · price** as the three columns every catalogue keeps — "the cart, the storefront and the
     * export all read it" — and this bootstrap declared only two of them. So `unit` arrived on every product,
     * was stored, exported and templated, and existed in no declaration: it showed up as an UNDECLARED column
     * on a brand-new entity that had done nothing wrong. Two files disagreeing about what is mandatory, and
     * nothing able to see it until the declaration and the store were compared.
     *
     * ⚠️ `quantity` WAS THE OPPOSITE PROBLEM, AND IS GONE (Q6, decided 2026-09-03). It was declared here as a
     * REQUIRED column while csv-preflight.BLOCKED refused it outright — *"quantity is what a customer orders,
     * not something a product carries"* — and validateAgainst skipped it. A required column no import may fill
     * and no validator checks only ever confused. It became urgent the moment the product form started
     * rendering declared columns: it would have appeared as a mandatory input nothing may satisfy.
     * Existing schemas lose it through migration b199 (only where no product ever recorded a value in it).
     */
    await query(
