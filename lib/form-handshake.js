...
function normalise(source) {
  return { key: String((source && source.key) || 'source'), label: String((source && source.label) || (source && source.key) || 'Source'),
...
function coverage(formSchema, sources) {
  ...
  const list = (Array.isArray(sources) ? sources : []).map(normalise);
...
function summarise(cov, sources) {
  const names = (Array.isArray(sources) ? sources : []).map(normalise).map((s) => s.label).join(' + ') || 'no source';
