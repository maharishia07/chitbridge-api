...
/**
 * diff(items, products, add) — PURE. items: the published copy; products: the brand's active rows [{ item_id, item_data }];
 * add: brand item ids to share for the first time. → { items, changes, candidates, missing }
 */
function diff(items, products, add) {
  ...
  return { items: out, changes, candidates, missing };
}
...
async function memberAdoptions(brand_id, source_key) {
  const { flags } = await net.flagsOf(brand_id);
  const out = [];
  for (const store_id of Object.keys(flags.network_members || {})) {
...
async function publish(brand_id, source_key, opts) {
  ...
  const d = diff(src.items || [], await brandProducts(brand_id), o.add);
...
async function brandView(brand_id) {
  ...
  return sources.map((s) => {
    ...
    return { source_key: s.source_key, title: s.title, items: (s.items || []).length,
             changes: d.changes, candidates: d.candidates, missing: d.missing,
...
async function storeNotices(store_id, adoptions) {
  ...
  for (const a of adoptions) {
    ...
    if (!s || !s.owner_entity_id || String(s.owner_entity_id) === String(store_id)) continue;
    ...
    for (const entry of log) {                                  /* newest first */
      for (const c of (entry.changes || [])) {
        ...
        out.push({ key, log_id: entry.id, source_key: a.source_key, brand_id: s.owner_entity_id, brand_name: brandName,
...
async function answer(store_id, { source_key, name, key, choice }) {
  if (['use', 'keep'].indexOf(choice) < 0) return { error: 'validation', message: 'choice must be use or keep' };
  if (choice === 'use') {
  ...
  if (key) await net.setIn(store_id, 'network_price_seen', key, choice === 'use' ? 'used' : 'kept');
...
module.exports = { keepFollow, FIELDS, suggested, fromProduct, diff, publish, cancel, apply, applyIfDue, brandView, storeNotices, answer };
