...
async function publish(brand_id, source_key, opts) {
  ...
  d.changes.push(...storeChanges(o.withdraw_at, o.restore_at, Object.keys(bf.network_members || {}),
...
async function brandView(brand_id) {
  ...
  const ids = Object.keys(flags.network_members || {});
  const names = ids.length ? await query('SELECT identity_id, display_name FROM identities WHERE identity_id = ANY($1::uuid[])', [ids])
    .catch(() => ({ rows: [] })) : { rows: [] };
  const stores = names.rows.map((r) => ({ id: String(r.identity_id), name: r.display_name || String(r.identity_id).slice(0, 8) }))
