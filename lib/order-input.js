...
// Reading a document into form fields. The VALUE patterns live here, keyed by a small vocabulary, so a declaration
// only ever names a label and a type — never a regex.
const SRC_MAX = 3;            // sources per item
const SRC_FIELD_MAX = 40;     // fields per source
const SRC_TYPES = ['number', 'string', 'code', 'year'];
...
function resolve(stored) {
  ...
  // SOURCES a document can be READ from (the handshake, wired to the storefront). Deliberately LABEL-ANCHORED, not
  // regex: a catalogue owner declares the text that precedes a value and its TYPE, and the reader builds the pattern
  // from an escaped literal. Accepting raw regex from a declaration would let a shop ship catastrophic backtracking
  // to its own customers' browsers, and would be impossible to review. Bounded in count and length for the same reason.
  const sources = Array.isArray(raw.sources) ? raw.sources.slice(0, SRC_MAX).map((s) => ({
    key:    String((s && s.key) || 'source').slice(0, 40),
    label:  String((s && s.label) || 'Document').slice(0, 120),
    accept: (Array.isArray(s && s.accept) ? s.accept : ['application/pdf']).filter((m) => DOC_MIME_ALLOW.includes(m)),
    fields: (Array.isArray(s && s.fields) ? s.fields.slice(0, SRC_FIELD_MAX) : []).map((f) => ({
      field: String((f && f.field) || '').slice(0, 60),
      after: String((f && f.after) || '').slice(0, 120),          // the literal label that precedes the value
      type:  SRC_TYPES.includes(f && f.type) ? f.type : 'string',
      to:    String((f && f.to) || (f && f.field) || '').slice(0, 60),   // which FORM field it lands on
    })).filter((f) => f.field && f.after && f.to),
  })).filter((s) => s.fields.length) : null;
  return { preset, pipeline: raw.pipeline === 'payload' || raw.pipeline === 'commerce' ? raw.pipeline : base.pipeline,
           showsPrice: base.showsPrice, negotiable: base.negotiable, schema, documents, sources, errors,
...
function forItem(catalogueInput, itemDecl) {
  ...
  // T3.2 · an item may NOT change the pipeline. The route branches on the SHOP's pipeline before it ever looks at an
  // item, so an item declaring a commerce preset on a payload catalogue did not switch paths — it merely re-imported
  // `quantity` and `price` into a form, letting a customer-supplied payload.price sit beside price:0/total:0 on the
  // chit. Self-contradictory, and exactly the sort of thing a sealed record must not contain.
  const merged = mergePatch({ preset: rawBase.preset, pipeline: rawBase.pipeline,
                              schema: rawBase.schema, documents: rawBase.documents }, itemDecl);
  const out = resolve(merged);
  if (out.pipeline !== catalogueInput.pipeline) {
    return { ...catalogueInput,
             errors: (catalogueInput.errors || []).concat(
               `item declaration would change the pipeline from "${catalogueInput.pipeline}" to "${out.pipeline}" — an item cannot do that; the catalogue decides`) };
  }
  return out;
}
...
/**
 * T3.8 · does the file actually LOOK like the type it claims? Magic bytes only — deliberately not a full sniffer.
 * A declared type that the bytes contradict makes the sealed "proof" a label rather than a fact.
 */
function magicMatches(mime, buf) {
  ...
  switch (mime) {
    ...
    case 'image/png':                           // 89 P N G \r \n 1A \n
      return b.length >= 8 && b[0] === 0x89 && b[1] === 0x50 && b[2] === 0x4E && b[3] === 0x47
          && b[4] === 0x0D && b[5] === 0x0A && b[6] === 0x1A && b[7] === 0x0A;
    default:
      return false;                             // an unrecognised type cannot be verified, so it is not accepted
  }
}
...
function validateDocuments(raw, decl, crypto, budget) {
  ...
  list.forEach((d, i) => {
    // T3.9 · sanitise the NAME at ingest. CRLF used to survive onto the chit and was only stripped at download —
    // one layer away from where it belongs. Control characters never belong in a filename we seal and echo back.
    const rawName = (d && d.name) ? String(d.name) : '';
    const label = (rawName.replace(/[ -]/g, '').trim().slice(0, 200)) || `document ${i + 1}`;
    ...
    // T3.11 · size-check BEFORE decoding. base64 is 4 chars per 3 bytes, so the decoded length is knowable from the
    // string. Decoding first meant a 5 MB Buffer was allocated and SHA-256'd before the limit could reject it, and
    // with gzip an attacker sent ~8 KB to make us hold ~22 MB across a DB round-trip.
    const b64 = String((d && d.data_base64) || '').replace(/^data:[^;]+;base64,/, '');
    const approxBytes = Math.floor(b64.length * 3 / 4);
    if (approxBytes > DOC_MAX_BYTES) { errors.push(`"${label}": larger than ${Math.round(DOC_MAX_BYTES / 1048576)} MB`); return; }
    if (b.bytes + total + approxBytes > DOC_MAX_TOTAL) { errors.push(`Documents total more than ${Math.round(DOC_MAX_TOTAL / 1048576)} MB per submission`); return; }
    ...
    // T3.8 · the declared MIME must match the BYTES. It was taken purely from the request, so an HTML file declared
    // as application/pdf was stored and sha256-sealed as a PDF — which makes "the filled form AND its proof" a label
    // rather than a fact, and hands an unverified type to any downstream/ERP consumer. Buffer.from(…,'base64') is
    // also lenient enough that "!!!!not base64%%%%" decodes to arbitrary bytes, so this catches that too.
    if (!magicMatches(mime, buffer)) {
      errors.push(`"${label}": the file does not look like ${mime} — declared type must match the actual bytes`); return;
    }
    total += buffer.length;
