...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, asOwner }) {
   ...
   */
  const offers = await liveOffers({ entity_id: entity.identity_id, withEntity });
  /* ⭐ THE TAX A CUSTOMER WILL PAY, per item. Athi, 2026-09-04: "it is not showing the tax to be applied" — the basket
     showed offers but no GST because the storefront payload never carried a rate. Same shelf (own + governed slabs ·
     categories · face) and the same resolver as the order path (routes/catalogue reprice → withTax), so the basket's
     GST equals the invoice's. Never fails the storefront: no shelf → no tax field, nothing else changes. */
  let taxOf = () => null;
  try {
    const taxShelf = require('./tax-shelf'), taxSlab = require('./tax-slab');
    const shelf = await taxShelf.readShelf(entity.identity_id, { withEntity, query, regionLayer: require('./regional').regionLayer, getFace: (eid) => getFace({ entity_id: eid, withEntity }) });
  ...
  /* the live CATEGORIES, for the crumb a customer sees above a product (name + parent walk) — same shape of read as offers */
  let categories = [];
  try {
    const rc = await withEntity(entity.identity_id, (db) => db.query(
      `SELECT d.definition_id, d.name, v.rules FROM definition d
        ...
        WHERE d.entity_id = $1 AND d.kind = 'category' AND d.status = 'live' ORDER BY d.name`, [entity.identity_id]));
    categories = rc.rows.map((d) => ({ id: d.definition_id, name: d.name, parent: (d.rules && d.rules.parent) || null }));
  } catch (_) { categories = []; }
