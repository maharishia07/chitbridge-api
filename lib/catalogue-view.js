...
async function getFace({ entity_id, withEntity }) {
  try {
    /* ⭐ ONE network round trip when the db module offers it (readBatch); the injected withEntity stays the fallback,
       so a caller that passes a transaction client of its own is unchanged. Read on every product page open. */
    const sql = `SELECT face FROM catalogue_face WHERE entity_id = $1`;
    let r = null;
    try { const db = require('../db'); if (typeof db.readBatch === 'function') r = (await db.readBatch(entity_id, null, [{ text: sql, params: [entity_id] }]))[0]; } catch (_) { r = null; }
    if (!r) r = await withEntity(entity_id, (db) => db.query(sql, [entity_id]));
...
/**
 * liveOffers — the offers a customer may be promised: LIVE, and not customer-group scoped. ONE reader for the
 * storefront payload AND the order path (routes/catalogue.js applies them to a shop order with the same engine).
 * No offers table, or the migration not run → [] — a catalogue without offers is the ordinary case, never a failure.
 */
/** ⭐ ONE MAPPING from a stored offer row to what the engine reads — liveOffers and the network's pinned versions both use it */
function offerFromRow(d) {
  const rules = (d.rules && typeof d.rules === 'object') ? d.rules : {};
  const alias = rules.alias != null ? String(rules.alias).trim() : '';
  const o = Object.assign({}, rules, { id: d.definition_id, label: alias || d.name, kind: d.sub_kind });
  delete o.alias;
  return o;
}
async function liveOffers({ entity_id, withEntity, viewer, groups, all }) {
  try {
    const r = await withEntity(entity_id, (db) => db.query(
      `SELECT d.definition_id, d.name, d.sub_kind, v.rules
         FROM definition d
         LEFT JOIN definition_version v
           ON v.definition_id = d.definition_id AND v.version = d.current_version
        WHERE d.entity_id = $1 AND d.kind = 'offer' AND d.status = 'live'
        ORDER BY d.name`, [entity_id]));
    /* ⭐ THE ALIAS IS THE ONLY NAME THAT LEAVES THE SHOP (Athi, 2026-09-06 19:2x: "inside he may have tier1, tier2, tier3, but outside everyone
       sees it as tier1 only"): rules.alias ("Shown to customers as") replaces the offer's name on every customer-facing surface — the
       storefront, the Suppliers screen, the list tag, the recorded line. The internal name stays in Offers. */
    const offers = r.rows.map(offerFromRow).filter((o) => o.kind);
    if (all) return offers;   /* the caller filters with groups it read in the same round trip (buildPublicView) */
    /* ⭐ CUSTOMER-ONLY OFFERS (Athi, 2026-09-06: "each customer gets a personalised discount… through the Suppliers menu"): an offer
       "Only for" a group or one customer reaches a viewer the SELLER's customer list places there (lib/customer-groups). No viewer
       — the public storefront — sees none of them: a promise to a stranger is a broken commitment, not a missed one. */
    const cg = require('./customer-groups');
    const gs = Array.isArray(groups) ? groups : await cg.groupsOf({ seller_id: entity_id, viewer_id: viewer, withEntity });
    return cg.offersFor(offers, gs);
  } catch (_) { return []; }
}
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, viewer_id, asOwner }) {
  ...
  /* the OWNER previewing their own shop passes the gate; the answer carries the real visibility so the page can say so */
  if (visibility === 'private' && !asOwner) return { available: false, reason: 'private' };

  /**
   * ⭐⭐ EVERYTHING AFTER THE GATE STARTS TOGETHER (2026-09-07, measured: this page cost 15 database trips and 1.5–1.8 s, and it is the
   * first thing a customer sees). None of these reads depends on another — the schema, the face, the items, the policy, the adoptions,
   * the storefront mode — and awaiting them in a line paid the distance to the database once per read.
   * ⚠️ THE GATE ABOVE STAYS ALONE: a private shop must be refused before anything of its rows is read.
   * ⚠️ applyDue is CHAINED to the items, not run beside them: a parked price change must land before the read that shows prices.
   */
  const _pSchema = query(
    `SELECT schema_id, schema_name FROM entity_schemas
     WHERE entity_id = $1 AND status = 'active' AND is_default = true${asOwner ? '' : " AND visibility = 'public'"} LIMIT 1`,
    [entity.identity_id]).then(async (sch) => {
      const rows = sch.rows.length
        ? (await query(
        ...
        : [];
      return { sch, fields: rows };
    });
  const _pFaceEarly = getFace({ entity_id: entity.identity_id, withEntity });
  const _pItems = require('./schedule').applyDue(entity.identity_id)
    .catch(() => {})                                   /* a schedule that cannot run must never stop a shop from opening */
    .then(() => withEntity(asOwner ? entity.identity_id : null, (db) => db.query(
      `SELECT item_id, item_data, created_at FROM catalogue_items
       WHERE entity_id = $1 AND is_active = true ORDER BY created_at DESC`, [entity.identity_id])));
  /* ⚠️ these three were already parallel (2026-09-05); they simply start earlier now, with the rest */
  const _pPolicy = policy.get(entity.identity_id).catch(() => ({}));
  const _pAdo = withEntity(entity.identity_id, (db) => db.query(
      `SELECT source_key, commercials FROM catalogue_adoption WHERE entity_id = $1 AND visible = true`, [entity.identity_id])).catch(() => ({ rows: [] }));
  const _pSf = query('SELECT storefront_access, policy_flags FROM identities WHERE identity_id = $1', [entity.identity_id]).catch(() => ({ rows: [] }));

  const { sch, fields } = await _pSchema;
  ...
  const _face = await _pFaceEarly;
  const _oiEarly = orderInputOf(_face, orderInput);
  const items = (await _pItems).rows;
  ...
  /* the policy flag, the adopted catalogues and the storefront mode were started with the wave above (2026-09-07) */
  let qtyZeroHides = false;
  try { qtyZeroHides = (await _pPolicy).qty_zero_hides === 'on'; } catch (_) {}
  ...
  let finishes = [];
  try {
    const ado = await _pAdo;
    /* the adoptions resolve together, not one after another */
    const resolvedAll = await Promise.all(ado.rows.map((row) => catalogueBuild.resolve(row.source_key, row.commercials || {}, { entity_id: entity.identity_id }).catch(() => null)));
    for (let ri = 0; ri < ado.rows.length; ri++) {
      const row = ado.rows[ri], resolved = resolvedAll[ri];
  ...
  let storefront_access = 'browse', upi_id = null, entityFlags = {};
  try {
    const sf = await _pSf;
    if (sf.rows[0] && sf.rows[0].storefront_access) storefront_access = sf.rows[0].storefront_access;
    /* ⭐ THE UPI ID (payment loop, level 1): the profile map's copy of it rides the same identities row — no extra trip; a
       storefront shows a QR on "Order placed" only when the store has one on record. Only the id travels, nothing else. */
    try { const pf = (sf.rows[0] && sf.rows[0].policy_flags) || {}; entityFlags = pf; upi_id = require('./profile').upiOf(pf);   /* ⭐ one definition of a valid payee address — see lib/profile.isUpiId */ } catch (_) {}
  } catch (_) {}

  /**
   * ⭐⭐⭐ [REV-04] THE COST STRIP, MOVED HERE — BEFORE lines[] IS BUILT, NOT AFTER (external code review,
   * 2026-09-25, §4: "cost price reaches the anonymous storefront"). catalogueRead.lines()'s ownedLine() copies
   * every key of item_data — cost included — into line.fields, by value, at the moment it runs. The exposure
   * strip further down in this function (still there, on `items` itself — this is additive, not a
   * replacement) runs 90+ lines later: it clears item_data.cost on the ORIGINAL item, but allLines' own copy
   * had already been taken, cost and all, and nothing about stripping `items` afterward reaches a copy that
   * was made before the strip ran. Proved against the real modules, in the real order, before this fix:
   *   items[0].item_data.cost = undefined   (the later strip worked, on `items`)
   *   lines[0].fields.cost    = {"value":380,...}   (allLines never heard about it)
   * Running the SAME exposureOf()/apply() pair here, before `items` is ever handed to catalogueRead.lines(),
   * means a line's `fields` can never hold a key the item itself is not allowed to show — there is no copy
   * left to leak from. [[feedback-search-before-you-build]]
   */
  try { const X = require('./exposure'); for (const it of items) X.apply(it, X.exposureOf(entityFlags, it.item_data)); } catch (_) {}
  ...
  /**
   * ── ⭐⭐ THE OFFERS TRAVEL WITH THE CATALOGUE ─────────────────────────────────────────────────────────────────
   *
   ...
   *
   * ⚠️⚠️ THIS READ HAS NEVER SENT THEM, so a discount was invisible to every buyer. `CBOffers.evaluate` runs on a
   * CART, in the signed-in app — the storefront, the B2B supplier view and a network peer were never given the
   * offers at all, and so could not have shown one if they had wanted to. A seller whose method of pushing
   * product IS the discount had that method switched off.
   *
   * ⭐ ONE READ, THREE SURFACES. buildPublicView already serves all three, so carrying offers here lights up the
   * storefront, the supplier view and the peer view together — the same reason the line read lives here.
   *
   * ⚠️ ONLY WHAT CAN BE HONESTLY ADVERTISED TO THIS VIEWER. An offer scoped to a customer_group cannot be
   * promised to an anonymous shopper — it would appear on the row and be declined at checkout, which is a broken
   * commitment rather than a missed one. Athi, 2026-09-03: *"we cannot establish relationship per entity… as long
   * as the offer is given, it has to be visible everywhere"* — so a group-scoped offer is simply not public, and
   * the extra a seller chooses to give one customer stays a business act rather than a published rule.
   *
   * ⚠️ LIVE ONLY. A draft offer is one you are still writing; a retired one was withdrawn. Neither is a promise.
   */
  /* ⚠️ THREE INDEPENDENT READS, ONE ROUND TRIP EACH ACROSS THE PACIFIC (sfo ↔ Mumbai ≈ 1.2 s a trip). Offers, the tax
     shelf and the categories were awaited one after another — ~3.6 s of a ~9 s storefront open (2026-09-05, 30 products,
     and the same for 2). They do not depend on each other, so they go together. The region is still the real fix. */
  const taxShelf = require('./tax-shelf'), taxSlab = require('./tax-slab'), customerGroups = require('./customer-groups');
  /* ⭐ PRESENCE, MEASURED (Athi, 2026-09-06: "if the connector is not responding, can we set the shop as away so the customer does not think
     the outcome is immediate?"). The owner's Open/Closed/Away stays THEIR word; this is the kit's last heartbeat, read in the same
     parallel batch (no extra trip). The storefront turns it into one sentence when the books have been silent for 20 minutes. */
  const [offersAll, shelf, rc, presence, viewerGroups, networkOffers] = await Promise.all([
    liveOffers({ entity_id: entity.identity_id, withEntity, all: true }),
    taxShelf.readShelf(entity.identity_id, { withEntity, query, regionLayer: require('./regional').regionLayer, getFace: (eid) => getFace({ entity_id: eid, withEntity }) }).catch(() => null),
    withEntity(entity.identity_id, (db) => db.query(
      `SELECT d.definition_id, d.name, v.rules FROM definition d
         LEFT JOIN definition_version v ON v.definition_id = d.definition_id AND v.version = d.current_version
        WHERE d.entity_id = $1 AND d.kind = 'category' AND d.status = 'live' ORDER BY d.name`, [entity.identity_id])).catch(() => ({ rows: [] })),
    query(`SELECT MAX(last_seen) AS last_seen, COUNT(*)::int AS n FROM identities WHERE parent_entity_id = $1 AND identity_type = 'actor' AND connector_type IS NOT NULL AND break_status = 'active'`, [entity.identity_id]).then((r) => r.rows[0] || null).catch(() => null),   /* ⚠️ a double comma here once made an EMPTY SLOT — the groups landed in a 6th position nobody read (2026-09-06, found by [OFF-03]) */
    customerGroups.groupsOf({ seller_id: entity.identity_id, viewer_id: viewer_id || null, withEntity }),
    /* ⭐⭐ THE NETWORK'S OFFERS, ON A MEMBER STORE'S STOREFRONT (2026-09-17). The counter already honoured what the brand released
       and the store took; the storefront showed the same products without them. Same reader (lib/network-offers), the store's
       own choices from the flags this read already holds; each adopted item gets the line_id the offers are aimed at. */
    require('./network-offers').forFinishes(entity.identity_id, finishes, { choices: (entityFlags && entityFlags.network_choices) || {} }).catch(() => [])
  ]);
  /* ⭐ only what can be honestly advertised to THIS viewer (customer-groups.offersFor): public offers for everyone, customer-only ones for the customers the seller named */
  const offers = customerGroups.offersFor(offersAll, viewerGroups).concat(networkOffers || []);
  /* ⭐ THE TAX A CUSTOMER WILL PAY, per item (Athi, 2026-09-04: "it is not showing the tax to be applied") — same shelf and
     resolver as the order path, so the basket's GST equals the invoice's. No shelf → no tax field, nothing else changes. */
  let taxOf = () => null;
  if (shelf) taxOf = (item_data) => { try { const r = taxSlab.resolve({ item_data, face: shelf.face, slabs: shelf.slabs, categories: shelf.categories }); return (r && r.rate != null) ? { rate: r.rate, cess: r.cess || 0, name: r.name || null, scheme: r.scheme || 'GST' } : null; } catch (_) { return null; } };
  /* the live CATEGORIES, for the crumb a customer sees above a product (name + parent walk) */
  const categories = (rc.rows || []).map((d) => ({ id: d.definition_id, name: d.name, parent: (d.rules && d.rules.parent) || null }));

  /* each item carries the tax it resolves to (rate · cess · name); nothing else about the item changes */
  try { for (const it of items) { const t = taxOf(it.item_data || {}); if (t) it.tax = t; } } catch (_) {}
  /* ⭐ WHAT A CUSTOMER MAY SEE, per item (lib/exposure.js): the business's defaults, the item's own switches — applied HERE, the one
     projection every surface reads, so the storefront, the Suppliers screen and the API obey without knowing the rule.
     ⚠️ [REV-04] KEPT DELIBERATELY, EVEN THOUGH allLines' OWN copy is now stripped earlier (see the comment there) —
     `items` itself still carries taxOf()'s output (it.tax, set two lines above) and anything else appended to it
     between here and the earlier strip, so this stays as the guard for THIS array. apply() is idempotent; running
     it twice on an already-stripped item changes nothing. */
  try { const X = require('./exposure'); for (const it of items) X.apply(it, X.exposureOf(entityFlags, it.item_data)); } catch (_) {}
  return {
    available: true, visibility, categories,
    shop: {
      ...
      business_status: entity.business_status || 'open', storefront_access, upi_id,
      /**
       * ⭐⭐ WHAT EACH UNIT ALLOWS, so the cart can obey it without a second call. Per unit:
       * { decimals, tol, min_qty } — may this be halved, how far above a whole number still counts as that
       * whole number, and the quantity from which that applies.
       * ⚠️ IT TRAVELS WITH THE CATALOGUE ON PURPOSE. A buyer browsing a supplier's catalogue must obey the
       * SELLER's rules, not their own — the same reason the seller's trade rides on a chit rather than being
       * looked up (a receiver cannot read a supplier's profile, and correctly so).
       * ⚠️ Absent means undeclared, not 'no decimals': the cart falls back to the unit's kind.
       */
      unit_rules: (entityFlags && entityFlags.unit_rules) || {},
      /* the reader's standing with this seller (customer-groups): the cart's engine needs it to honour a customer-only offer's condition; [] for a stranger */
      viewer_groups: viewerGroups,
      /* the books' presence: { last_seen, silent_minutes } when a connector exists, else null — the storefront's 'away' sentence reads silent_minutes */
      connector: (presence && presence.n > 0) ? { last_seen: presence.last_seen, silent_minutes: presence.last_seen ? Math.round((Date.now() - new Date(presence.last_seen).getTime()) / 60000) : null } : null,
    ...
    fields,
    /* ⭐ The live offers, so a ROW can say what it promises before anything is in a basket. See
       CBOffers.forLine / promise on the client — the terms travel, the arithmetic stays where the cart is. */
    offers,
    items,
    // ── THE ONE READ ─────────────────────────────────────────────────────────────────────────────────────────
...
module.exports = { offerFromRow, liveOffers, buildPublicView, getFace, orderInputOf, getOrderInput, catalogueVisibility };
