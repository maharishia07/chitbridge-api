 ...
 */
async function catalogueVisibility({ entity_id, query, visibilityCap }) {
  try {
    ...
    const r = await query(
      'SELECT catalogue_visibility, plan, params_override FROM identities WHERE identity_id = $1', [entity_id]);
    const row = r.rows[0];
    if (!row) return null;
    const v = row.catalogue_visibility;
    const chosen = (v === 'public' || v === 'private') ? v : null;
    if (!visibilityCap) return chosen;                       // not injected → pre-cap behaviour, unchanged
    const cap = visibilityCap.capOf({ plan: row.plan, paramsOverride: row.params_override || {} });
    if (cap.max === 'private' && chosen === 'public') return 'private';   // the cap wins, silently and closed
    return chosen;
  } catch (_) { return null; }   // b114 not applied → pre-b114 behaviour
}

async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap }) {
  ...
  const visibility = await catalogueVisibility({ entity_id: entity.identity_id, query, visibilityCap });
