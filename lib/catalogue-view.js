...
async function catalogueVisibility({ entity_id, query, visibilityCap, viewer }) {
  try {
    ...
    const r = await query(
      'SELECT catalogue_visibility, plan, params_override, bridge_id FROM identities WHERE identity_id = $1', [entity_id]);
    const row = r.rows[0];
    if (!row) return null;
...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer }) {
  ...
  /**
   * ⚠️ PROVISIONAL ITEMS ARE NOT ON THE SHOP FRONT. A row created because a customer asked for something you do
   * not stock (see capture.raisePayload) exists so YOU can see the demand and price it — it is not an offer, and
   * it has no price. On the public storefront it would read as "we sell this, at nothing".
   *
   * This NARROWS what the storefront shows. Nothing that was public stops being public: existing rows carry no
   * `provisional` key, so the filter is false for every one of them.
   */
  const items = (await withEntity(null, (db) => db.query(
    `SELECT item_id, item_data, created_at FROM catalogue_items
     WHERE entity_id = $1 AND is_active = true
       AND COALESCE((item_data->>'provisional')::boolean, false) = false
     ORDER BY created_at DESC`, [entity.identity_id]))).rows;
   ...
   */
  const _oiEarly = await getOrderInput({ entity_id: entity.identity_id, withEntity, orderInput });
