...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, asOwner }) {
  ...
  let storefront_access = 'browse', upi_id = null;
  try {
    const sf = await _pSf;
    ...
    try { const pf = (sf.rows[0] && sf.rows[0].policy_flags) || {}; const u = pf.profile_provenance && pf.profile_provenance.upi_id && pf.profile_provenance.upi_id.value; if (u && /^[\w.\-]{2,}@[\w\-]{2,}$/.test(String(u))) upi_id = String(u); } catch (_) {}
  } catch (_) {}
