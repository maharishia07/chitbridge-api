...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, asOwner }) {
  ...
  let offers = [];
  try {
    const r = await withEntity(entity.identity_id, (db) => db.query(
      `SELECT d.definition_id, d.name, d.sub_kind, v.rules
        ...
        ORDER BY d.name`, [entity.identity_id]));
    offers = r.rows
    ...
    /* No offers table yet, or the migration has not run — a catalogue without offers is the ordinary case, not a
       failure, and it must never take the storefront down with it. */
    offers = [];
  }


  return {
    available: true, visibility,
    shop: {
