...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, viewer_id, asOwner }) {
  ...
  await require('./schedule').applyDue(entity.identity_id);   /* a parked change whose moment has come lands before the storefront reads */
  const items = (await withEntity(asOwner ? entity.identity_id : null, (db) => db.query(
  ...
  /* ⚠️ THREE MORE INDEPENDENT ROUND TRIPS (2026-09-05, the storefront opened in ~7 s): the policy flag, the adopted
     reference catalogues and the storefront access mode do not depend on each other — they START here, together, and
     are awaited where each is used. The region is still the real fix. */
