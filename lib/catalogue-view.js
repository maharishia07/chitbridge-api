 ...
 */
async function liveOffers({ entity_id, withEntity }) {
  try {
    ...
    return r.rows
      ...
      .filter((o) => o.kind && !o.customer_group);
  } catch (_) { return []; }
}
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, asOwner }) {
  ...
  const taxShelf = require('./tax-shelf'), taxSlab = require('./tax-slab');
  ...
  const [offers, shelf, rc, presence] = await Promise.all([
    liveOffers({ entity_id: entity.identity_id, withEntity }),
