...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity, catalogueRead, container, visibilityCap, viewer, viewer_id, asOwner }) {
  ...
  const [offersAll, shelf, rc, presence, viewerGroups] = await Promise.all([
    ...
    query(`SELECT MAX(last_seen) AS last_seen, COUNT(*)::int AS n FROM identities WHERE parent_entity_id = $1 AND identity_type = 'actor' AND connector_type IS NOT NULL AND break_status = 'active'`, [entity.identity_id]).then((r) => r.rows[0] || null).catch(() => null),,
