...
async function buildPublicView({ entity, query, withEntity, catalogueBuild, orderInput, identity }) {
  ...
  try {
    ...
    if (ident.variants) {
      groups = identity.group(items.map((r) => Object.assign({ _item_id: r.item_id }, r.item_data)), ident)
        .map((g) => ({
