...
async function deliver(sender_entity_id, chit_id, copies, opts = {}) {
  ...
  if (addressed) {
    const away = (copies || []).filter((c) => {
      const a = addressed.get(String(c.entity_id));
      return a && a.local === false;
    });
    if (away.length) {
      const e = new Error('CTP is not built yet: ' + away.length + ' of ' + copies.length
        + ' copies belong to an installation hosted elsewhere ('
        + [...new Set(away.map((c) => (addressed.get(String(c.entity_id)) || {}).installation_key))].join(', ')
        + '). Nothing was delivered.');
      e.code = 'CTP_NOT_BUILT';
      throw e;                             /* ⚠️ before any write — all or none */
    }
  }
