...
// commerceStandards() — the commerce standards (Incoterms/UCP/FRM) as they now live in the governed source-entity table
// (seeded by b93, facet 'commerce'), each with its SEALED owner source-entity. This is the proof that the commerce layer
// rides the same DDL machinery as ISO 9001 — the code catalogue above is merely their operational face.
async function commerceStandards() {
  try {
    const { query } = require('../db');
    const r = await query(
      `SELECT s.standard_key, s.title, s.facet, s.guidance,
              i.bridge_id AS owner_bridge_id, i.display_name AS owner_name, i.sealed AS owner_sealed
         FROM standard_source s
         LEFT JOIN identities i ON i.identity_id = s.owner_entity_id
        WHERE s.facet = 'commerce' AND s.active = true
        ORDER BY s.standard_key`);
    return { framework: 'FRM', governed: true, sources: r.rows };
  } catch (_) { return { framework: 'FRM', governed: true, sources: [] }; }
}

module.exports = { resolveInstruments, resolveJourney, commerceStandards, RISKS, FRM_CLASSES, INSTRUMENTS, PARTNER_TYPES, STAGES };
