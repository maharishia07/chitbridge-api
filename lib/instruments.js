...
const RISKS = {
  payment:     { label: 'Payment risk',      q: 'Will the buyer actually pay?' },
  performance: { label: 'Performance risk',  q: 'Will the seller deliver as promised?' },
  transit:     { label: 'Transit / cargo risk', q: 'What if the goods are lost or damaged in transit?' },
  currency:    { label: 'Currency risk',     q: 'What if the exchange rate moves before settlement?' },
  price:       { label: 'Price / commodity risk', q: 'What if the input/commodity price swings?' },
  country:     { label: 'Country / political risk', q: 'What if the buyer\'s country blocks payment or import?' },
  liquidity:   { label: 'Liquidity risk',    q: 'How does the seller fund the gap until payment lands?' },
};
...
function resolveInstruments({ cross_border = true, incoterm } = {}) {
  ...
  const cluster = Object.keys(RISKS).map(risk => {
    ...
    return { risk, label: RISKS[risk].label, question: RISKS[risk].q, covered_onrail: items.some(i => i.onrail === 'built'), instruments: items };
  }).filter(g => g.instruments.length);
  return { cross_border: !!cross_border, incoterm: ic, cargo_insured_by: insures, cluster };
}
...
module.exports = { resolveInstruments, resolveJourney, RISKS, INSTRUMENTS, PARTNER_TYPES, STAGES };
