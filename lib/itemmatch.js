...
/**
 * ── ⭐ stampOf(it) — WHAT THIS ITEM WAS AT THIS MOMENT ───────────────────────────────────────────────────────────
 *
 ...
 * the amendment, capture, chit details. That is the total crux of the product."*
 *
 * An id alone does not fix the base. `item_id` says WHICH row; the row keeps moving — reprice it, rename it, take
 * it out of stock — so a chit holding only an id points at something that is no longer what was ordered, and
 * nothing anywhere says it changed. The reference has to carry its own moment.
 *
 *   as_of — when the row last changed, as the catalogue itself recorded it
 *   hash  — what it SAID at that moment
 *
 * Together they make drift detectable instead of invisible: the chit says this item hashed to H; the catalogue row
 * today hashes to something else; that difference is a fact anyone can check, months later, without a version
 * table and without trusting either party's memory.
 *
 * ⚠️ A FIXED TUPLE OF SCALARS, NOT CANONICALISED JSON — deliberately. Hashing free-form jsonb means agreeing
 * forever on key order, unicode form and nesting depth, and this codebase already carries hand-rolled
 * canonicalisers that had to be depth-capped. Six named fields in a fixed order cannot drift, cannot nest, and
 * mean the same thing to a reader in five years. If a seventh field ever matters, adding it changes every hash —
 * which is exactly why the list is short and explicit rather than "whatever was on the row".
 *
 * ⚠️ AND IT IS NOT A SEAL. This is evidence that the base moved, not a cryptographic commitment by either party —
 * the chit's own seal is what makes the line itself tamper-evident.
 */
/* ⚠️ NUL, WRITTEN AS AN ESCAPE. A space is a character these fields legitimately contain, so joining on one
   would let {name:'Tomato Hybrid', variant:''} and {name:'Tomato', variant:'Hybrid'} hash identically — two
   different rows producing one stamp, the single thing a stamp must never do. Kept as \u0000 rather than a raw
   byte: a literal NUL in a source file makes it read as binary to grep and every other text tool. */
const SEP = '\u0000';
const STAMP_FIELDS = ['name', 'variant', 'unit', 'price', 'sku', 'status'];
function stampOf(it) {
  const money = require('./money');
  const parts = STAMP_FIELDS.map((f) => {
    /**
     * ⚠️ amountOfLoose, NOT amountOf — and the difference is the raise path staying up. amountOf THROWS on a
     * legacy bare price with no currency ("Legacy price 36 has no currency…"), which is a correct refusal when
     * money is about to be committed and a catastrophic one here: a hash is evidence, not a transaction, and a
     * stamping helper that can abort would take the whole capture down for any catalogue not yet migrated.
     * The currency is uniform within one entity's catalogue, which is exactly amountOfLoose's stated condition.
     *
     * ⚠️ AND NaN STRINGIFIES TO "NaN" DELIBERATELY — an unreadable price is a real state of the row, and two rows
     * that both have no readable price genuinely did say the same thing.
     */
    const v = (f === 'price') ? money.amountOfLoose(it.price) : it[f];
    return v === null || v === undefined ? '' : String(v);
  });
  return require('crypto').createHash('sha256').update(parts.join(SEP), 'utf8').digest('hex').slice(0, 16);
}
...
async function loadCatalogue(entity_id) {
  ...
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      /* ⚠️ item_id IS SELECTED NOW, and its absence was the reason a chit could not carry a durable reference at
         all. The matcher could only ever hand back what it read out of item_data, so the strongest identity it
         could offer was a name — which is precisely the thing that changes. */
      `SELECT item_id, item_data, updated_at FROM catalogue_items WHERE entity_id = $1 AND is_active = true`, [entity_id]));
    rows = r.rows.map((x) => Object.assign({ _item_id: x.item_id, _as_of: x.updated_at }, x.item_data || {})).filter((d) => d && d.name);
  } catch (_) { rows = []; }          // no catalogue is not an error — it means nothing gets normalised or priced

  /**
   * ⭐ b146 — the CURRENT version number per item, read SEPARATELY and on purpose.
   *
   * ⚠️ NOT A JOIN ON THE QUERY ABOVE. That query's catch sets `rows = []`, which is right for "this entity has no
   * catalogue" and catastrophic for "the version table does not exist yet": every item would vanish, every line
   * would come back unmatched and unpriced, and the only symptom would be a shop whose catalogue silently stopped
   * working the moment the code deployed ahead of the migration. Deploy order is a fact of life here — the API
   * ships before Athi runs the SQL — so the new table must never be able to take the old path down.
   *
   * Pre-migration this fails, `version` stays null, and every reference falls back to `as_of` + `hash`, which is
   * the shape that already works.
   */
  const versions = new Map();
  try {
    const v = await withEntity(entity_id, (c) => c.query(
      `SELECT item_id, version_no FROM catalogue_item_version
        WHERE entity_id = $1 AND valid_to IS NULL`, [entity_id]));
    v.rows.forEach((x) => versions.set(String(x.item_id), x.version_no));
  } catch (_) { /* b146 not applied — the hash is the fallback, and it is a good one */ }
  ...
  const items = rows.filter((d) => itemstatus.isMatchable(d)).map((d) => ({
    ...
    /**
     * ── ⭐ THE DURABLE IDENTITY, AND WHY THERE ARE TWO OF THEM ────────────────────────────────────────────────
     * Athi, 2026-08-13: *"does the chit carry the sku reference? is it required, so next time when you look at
     * it you may be able to use it directly?"*
     *
     * ⚠️ `key` IS NOT AN IDENTIFIER. It is norm(name)|norm(variant) — derived from the very text that changes.
     * Rename a product and the key changes with it, so a chit keyed that way loses its link at exactly the moment
     * the link is needed. It stays as a within-request pick token and nothing more.
     *
     * ⚠️ TWO IDENTIFIERS BECAUSE THEY HAVE DIFFERENT AUDIENCES, which is Athi's own call on what may travel:
     *   item_id — the row's uuid. INTERNAL. Mine, meaningless to anyone else, stays on my copy.
     *   sku     — the PUBLISHED identifier, the one already printed on the storefront the buyer ordered from.
     *             That is the one a counterparty may legitimately hold, because they read it before they ordered.
     *
     * ⚠️ `code` IS DELIBERATELY NOT A FALLBACK, and it was in the first version of this line. The starter set
     * labels that column "Code / HSN", so it holds either a merchant SKU or a WCO customs classification with
     * nothing to say which — the exact conflict the standards audit recorded on 2026-08-13. Reading it as an
     * identifier would stamp an HS code onto a chit line as though it named the product, and it would look
     * completely right. A catalogue with no `sku` truthfully has no published identifier yet; the internal
     * item_id still works for our own copy, and the missing half is a reason to split that column, not to guess.
     */
    item_id: d._item_id || null,
    /* WHEN this row last changed, as the catalogue itself recorded it — half of 'what was it at that moment'. */
    as_of: d._as_of ? new Date(d._as_of).toISOString() : null,
    /* b146: authoritative WHEN APPLIED — a number both parties can name. Null before the migration. */
    version: versions.get(String(d._item_id)) || null,
 ...
 */
const candidate = (it) => ({
  key: it.key, item_id: it.item_id || null, sku: it.sku || null,
  name: it.name, variant: it.variant || '', unit: it.unit || '',
...
function match(phrase, comment, cat) {
  ...
  for (const it of named) {
    ...
    if (it.variant && it.synonyms.includes(p)) {
      ...
      if (carriers.length === 1) return { item: it, how: 'synonym' };
    }
    return decideVariant(norm(it.name), it, { how: norm(it.name) === p ? 'exact' : 'synonym' });
  }
  // 2 · the phrase carries the variant inline: "orange grade 1"
  for (const it of items) {
    if (!it.variant) continue;
    if (p === norm(it.name + ' ' + it.variant) || (p.startsWith(norm(it.name)) && p.includes(norm(it.variant)))) return { item: it, how: 'exact' };
  }
  ...
  if (nearNames.size === 1) return decideVariant([...nearNames][0], near[0], { how: 'contains' });
  ...
  /**
   * ⚠️⚠️ A TOLERANCE SET BY THE CANDIDATE'S LENGTH TURNED FOUR DIFFERENT PARTS INTO ONE.
   * Found 2026-08-30 from Athi's own WhatsApp test: "AC filter-um maathidunga" became an Oil filter line.
   *
   ...
   *
   * The old rule read tol from the CANDIDATE'S TOTAL LENGTH, so "Oil filter" (10 chars) earned a
   * tolerance of 3 — and because both strings share the long suffix "filter", the whole distance sits
   * in the short word that actually names the product. In a workshop those are four parts at four
   * prices. Edit distance cannot tell a typo from a different qualifier: "Oil fillter" is 1 edit and
   * "Air filter" is 2, so the WRONG PRODUCT scores worse than the typo and still passes.
   *
   * ⭐⭐ THE RULE IS NOW PER WORD. Same number of words on both sides means the difference must be a
   * misspelling INSIDE a word, never a whole word swapped. One-word phrases fall through to the old
   * whole-string rule untouched, so coolent->Coolant and batery->Battery still resolve.
   *
   * ⭐ AND A NEAR MISS IS NOT A DEAD END — Athi, 2026-08-30: "flag as ambiguous with candidates."
   * Anything the old rule would have taken is still PLAUSIBLE; it is just not proof. Those become
   * candidates on an ambiguous result, which capture.js already turns into needs_human with a picker
   * (lib/capture.js line ~327, rendered by cap-chit2.js). So the person is asked "you said AC filter,
   * the nearest thing we sell is Oil filter" instead of being handed the wrong part silently.
   */
  const tolFor = (s) => (s.length <= 4 ? 1 : s.length <= 7 ? 2 : 3);
  /* Plausible: the whole string is close. Enough to offer, not enough to choose. */
  const nearMiss = (a, b) => { const d = lev(a, b); return d <= tolFor(b) ? d : null; };
  /* A misspelling proper: word counts agree and every word clears ITS OWN tolerance. */
  const misspelling = (a, b) => {
    const at = a.split(' ').filter(Boolean), bt = b.split(' ').filter(Boolean);
    if (at.length !== bt.length || at.length < 2) return nearMiss(a, b);
    let total = 0;
    for (let i = 0; i < at.length; i++) {
      const d = lev(at[i], bt[i]);
      if (d > tolFor(bt[i])) return null;
      total += d;
    }
    return total;
  };
  ...
  for (const it of items) {
    ...
    for (const cand of [norm(it.name), ...it.synonyms]) {
      if (!cand) continue;
      if (nearMiss(p, cand) !== null) isNear = true;
      const d = misspelling(p, cand);
      if (d !== null && d < bestD) { bestD = d; best = it; }
    }
    if (isNear) plausible.push(it);
  }
  if (best) return decideVariant(norm(best.name), best, { fuzzy: bestD, how: 'fuzzy' });
  /* ⚠️ matches counts distinct NAMES, the same meaning the two ambiguity paths above use. */
  if (plausible.length) {
    return { ambiguous: true, matches: new Set(plausible.map((it) => norm(it.name))).size,
             candidates: plausible.map(candidate), how: 'near' };
  }

  return { unmatched: true, reason: 'no catalogue match' };
}

module.exports = { loadCatalogue, match, norm, lev, candidate, keyOf, stampOf, STAMP_FIELDS };
