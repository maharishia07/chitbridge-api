...
function match(phrase, comment, cat) {
  ...
  for (const it of items) {
    if (norm(it.name) === p || it.synonyms.includes(p)) return decideVariant(norm(it.name), it);
  }
  ...
  const near = items.filter((d) => { const dn = norm(d.name); return dn && (dn.includes(p) || p.includes(dn)); });
