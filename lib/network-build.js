  ...
  function walk(parentKey) {
    for (const n of childrenOf(parentKey)) {
      ...
      out.create.push({
        ...
        name,
        handle: made.handle,
        // parent_key drives PLACEMENT on the tree. There is deliberately no `parent_handle` — the handle is not
        // derived from the parent, and a field implying it was would be read as authority sooner or later.
