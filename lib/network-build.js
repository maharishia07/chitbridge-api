...
/**
 * Where a store sits among its siblings — the operator's deliberate arrangement (b118).
 *
 * `null` means "not arranged", which is a real answer and not a zero: those sort last, by name, so a network keeps
 * the order it has today until someone actually moves something.
 */
function orderOf(node) {
  const p = node && node.pos;
  return Number.isInteger(p) ? p : (typeof p === 'number' && isFinite(p) ? Math.round(p) : null);
}

/**
 * WHERE a store is (b119). Returns a normalised place, or null when nothing has been said.
 *
 * Coordinates are validated, not trusted: a latitude of 200 is not a place, and a design that carries one should
 * be refused a coordinate rather than be given a wrong one. Out-of-range values become null — the store keeps its
 * address and stays "not located", which is an honest state and the one nearest-store already handles.
 */
function placeOf(node) {
  const p = (node && node.place && typeof node.place === 'object') ? node.place : null;
  if (!p) return null;
  const txt = (v, max) => { const s = String(v == null ? '' : v).trim().slice(0, max); return s || null; };
  const num = (v, lo, hi) => {
  ...
  const out = {
    address: txt(p.address, 300),
    city: txt(p.city, 120),
    country: txt(p.country, 60),
    // The store's OWN currency, when it trades somewhere else. Null means "take the network's" — and it must stay
    // null rather than being pre-filled, or every store would look like it had decided something it never did.
    // Three letters, uppercase: a currency is an ISO code, and 'rupees' is not one.
    currency: (() => { const c = txt(p.currency, 8); return c && /^[A-Za-z]{3}$/.test(c) ? c.toUpperCase() : null; })(),
    tz: txt(p.tz, 60),
    // b120 — how quickly this store can send. DECLARED per store; null means not declared, and that must reach
    // the screen as such rather than being filled in with an average.
    dispatch_days:    (() => { const n = num(p.dispatch, 0, 365); return n === null ? null : Math.round(n); })(),
    ship_within_days: (() => { const n = num(p.within,   0, 365); return n === null ? null : Math.round(n); })(),
    ship_beyond_days: (() => { const n = num(p.beyond,   0, 365); return n === null ? null : Math.round(n); })(),
    lat: num(p.lat, -90, 90),
    lng: num(p.lng, -180, 180),
    // A radius is only meaningful with a point to centre it on, and 0 km is not a service area.
    service_km: (() => { const n = num(p.km, 1, 20000); return n === null ? null : Math.round(n); })(),
  };
  // ONE of the two coordinates is not a location. Half a point would place the store on a meridian.
  if (out.lat === null || out.lng === null) { out.lat = null; out.lng = null; }
  const said = Object.keys(out).some((k) => out[k] !== null);
  return said ? out : null;
}

/** Do two places differ? Field by field, so a corrected postcode is a change and a re-typed identical one is not. */
function placeSame(a, b) {
  const A = a || {}, B = b || {};
  return ['address', 'city', 'country', 'currency', 'lat', 'lng', 'service_km',
          'dispatch_days', 'ship_within_days', 'ship_beyond_days']
    .every((k) => (A[k] === null || A[k] === undefined ? null : A[k]) === (B[k] === null || B[k] === undefined ? null : B[k]));
}

/** One line saying what a store is FOR. Clamped to the column (varchar(200)) so a build can never be refused for it. */
const MAX_PURPOSE = 200;
function purposeOf(node) {
  return String((node && node.purpose) || '').trim().slice(0, MAX_PURPOSE);
}
  ...
  function walk(parentKey) {
    for (const n of childrenOf(parentKey)) {
      ...
      if (already) {
        ...
        const liveRow = liveBy ? liveBy[already.bridge_id] : null;
        const now = liveRow ? String(liveRow.catalogue_visibility || '') : null;
        const visMoved = now !== null && now !== want;
        // The PURPOSE travels the same way — it is on the store, so an edit to the drawing has to be applied like
        // any other change rather than silently diverging. Compared as trimmed strings; absent and '' are one thing.
        const wantPurpose = purposeOf(n);
        const nowPurpose = liveRow ? String(liveRow.purpose || '').trim() : null;
        const purposeMoved = nowPurpose !== null && nowPurpose !== wantPurpose;
        // The ARRANGEMENT travels too (b118). Without it, dragging a store into place would change the operator's
        // drawing and nothing else — the exact divergence between "what I see" and "what everyone else sees" that
        // the stored position exists to end.
        const wantOrder = orderOf(n);
        const nowOrder = liveRow ? (liveRow.sort_order === null || liveRow.sort_order === undefined ? null : Number(liveRow.sort_order)) : undefined;
        const orderMoved = nowOrder !== undefined && nowOrder !== wantOrder;
        // WHERE it is travels the same way (b119) — a corrected address that never reaches the store is a store
        // the map will keep sending people to the wrong place.
        const wantPlace = placeOf(n);
        // Every field placeSame() compares must be here, or an edit to one of them would never be noticed and the
        // design would drift from the store in exactly the way this whole convergence exists to prevent.
        const numOrNull = (v) => (v === null || v === undefined ? null : Number(v));
        const nowPlace = liveRow ? { address: liveRow.address, city: liveRow.city, country: liveRow.country,
                                     currency: liveRow.currency_code || null,
                                     lat: numOrNull(liveRow.lat), lng: numOrNull(liveRow.lng),
                                     service_km: numOrNull(liveRow.service_km),
                                     dispatch_days: numOrNull(liveRow.dispatch_days),
                                     ship_within_days: numOrNull(liveRow.ship_within_days),
                                     ship_beyond_days: numOrNull(liveRow.ship_beyond_days) } : null;
        const placeMoved = !!(liveRow && wantPlace && !placeSame(wantPlace, nowPlace));
        if (visMoved || purposeMoved || orderMoved || placeMoved) {
          const u = { key: n.key, name, handle: already.user_id, bridge_id: already.bridge_id };
          if (visMoved) { u.from = now; u.to = want; }
          if (purposeMoved) u.purpose = { from: nowPurpose, to: wantPurpose };
          if (orderMoved) u.order = { from: nowOrder, to: wantOrder };
          if (placeMoved) u.place = { from: nowPlace, to: wantPlace };
          out.update.push(u);
        } else {
      ...
      out.create.push({
        ...
        purpose: purposeOf(n),           // carried onto the store, so anyone who may see it can read why it exists
        sort_order: orderOf(n),          // b118 — the operator's arrangement, so every reader sees one order
        place: placeOf(n),               // b119 — where it is; 'closest store' is a distance question
      });
...
module.exports = { plan, visibilityOf, purposeOf, orderOf, placeOf, placeSame, EXPOSURE, MAX_PURPOSE };
