...
async function readShelf(entity_id, deps, opts) {
  try { await require('./schedule').applyDue(entity_id); } catch (_) {}   /* the send path reads the catalogue as it IS at this moment — parked changes due by now included */
  const d = deps || {}, o = opts || {};
  if (!entity_id || typeof d.withEntity !== 'function') return null;
  try {
    const [defs, face, gov, items] = await Promise.all([
      d.withEntity(entity_id, (db) => db.query(
      ...
      o.withItems
        ? d.withEntity(entity_id, (db) => db.query(
            `SELECT item_id, item_data FROM catalogue_items WHERE entity_id = $1 AND is_active = true`, [entity_id])).catch(() => ({ rows: [] }))
