...
/** How OPEN each tier is. A cap bounds this number; it never bounds how closed an entity may choose to be. */
...
function capOf(opts = {}) {
  ...
  /**
   * ⚠️ THE MIDDLE RUNG, ADDED 2026-08-07 WITH THE NETWORK BUILD — and it was a hole, not an omission.
   *
   * Until the build existed, nothing ever WROTE `caps.catalogue_visibility`, so a value of `network` here was
   * unreachable and this function only ever answered `private` or `public`. The build writes it on every minted
   * node: a warehouse designed as network-only is capped at `network`.
   *
   * Without this branch that declaration would have fallen straight through to "nothing declared → public", and a
   * warehouse minted as internal could have published itself to the world from its own Settings screen — while the
   * network page showed it as protected. Same species as the plan-menu scar above, arriving from the other
   * direction: there, a value nobody had audited became policy; here, a value we deliberately wrote would have
   * been silently ignored. A cap is not a cap until something enforces it.
   */
  if (opCap === NETWORK) {
    return { max: NETWORK, by: 'operator', enforced: true,
      reason: 'Your network operator set this entity to be visible to the network only. It cannot publish publicly.' };
  }
...
function check(want, cap) {
  ...
  //
  // Compared by RANK rather than equality, so the middle rung actually bounds anything: `cap.max === PRIVATE` was
  // the only refusal until the build started issuing `network` caps, and equality would have let a network-capped
  // entity choose `public` unchallenged.
  if (cap && RANK[cap.max] !== undefined && RANK[w] > RANK[cap.max]) {
...
function effective(chosen, cap) {
  ...
  if (RANK[c] === undefined) return PRIVATE;
  // The NARROWER of the two, by rank. Written as `cap.max === PRIVATE` until the middle rung existed, which would
  // have reported a network-capped entity's chosen `public` back as `public` — the profile screen agreeing with a
  // choice the platform would not honour.
  if (cap && RANK[cap.max] !== undefined && RANK[c] > RANK[cap.max]) return cap.max;
  return c;
}

module.exports = { capOf, check, effective, PUBLIC, PRIVATE, NETWORK, RANK };
