...
async function attachables() {
  if (!(await schema.hasTable('register_attachable'))) return [];
  try {
    ...
    return r.rows;
  } catch (e) { if (notMigrated(e)) return []; throw e; }
}
...
async function addTemplate(entity_id, opts = {}) {
  ...
  try {
    return await withEntity(entity_id, async (c) => {
      ...
      for (const st of (opts.standards || [])) {
        const key = typeof st === 'string' ? st : st.standard_key;
        const clause = typeof st === 'string' ? '' : (st.clause || '');
        if (!key) continue;
        await c.query(
          `INSERT INTO register_template_standard (entity_id, template_id, standard_key, clause)
           VALUES ($1,$2,$3,$4) ON CONFLICT DO NOTHING`, [entity_id, template_id, key, clause]);
      }
