...
/** Which shape this database is on. `full` means b185 has run. */
async function shape() {
  if (await schema.hasTable('register_subject')) return { table: 'register_entry', full: true };
  if (await schema.hasTable('register_entry')) return { table: 'register_entry', full: false };
  if (await schema.hasTable('chit_line_raida')) return { table: 'chit_line_raida', full: false };
  return { table: null, full: false };
}
...
async function attachables() {
  /**
   * ⚠️⚠️ NO hasTable() GUARD HERE, DELIBERATELY — it is what made this lie on 2026-08-30. `hasTable` reads
   * `information_schema.tables`, and that view SHOWS ONLY WHAT THE ROLE HOLDS A PRIVILEGE ON: a table that
   * exists but was never GRANTed to cb_app is indistinguishable from one that was never created. Both came back
   * as a bare `[]`, which reads as "nothing may be attached" — a silent wrong answer, the failure this file
   * exists to avoid. A direct SELECT with a TYPED catch tells the three cases apart, so it does, and says which.
   */
  try {
    ...
    return { attachables: r.rows, degraded: r.rows.length ? null : 'empty' };
  } catch (e) {
    /* Not there yet — deploy-before-migration, the normal window. */
    if (e.code === '42P01') return { attachables: [], degraded: 'absent' };
    /* ⭐ It IS there and we may not read it. Only a missing GRANT does this, and it is invisible from every other
       angle: a foreign key INTO this table still holds, because FK enforcement does not need SELECT. */
    ...
    return await onEntity(entity_id, opts.db, async (c) => {
      ...
      if (!subject_id) {
        const subj = await subjectFor(entity_id, {
          ...
          name: opts.subject_name || 'Order', ref_label: opts.subject_name || null }, c);
...
async function revise(entity_id, raida_id, fields = {}) {
  ...
  try {
    return await withEntity(entity_id, async (c) => {
      const t = await c.query(
        `SELECT chit_id, line_id, subject_id, kind, closes_id FROM register_entry
      ...
      const vals = [entity_id, E.chit_id, E.line_id, E.subject_id, E.kind,
                    fields.body || '(revised)', raida_id, reqctx.currentActor() || null, fields.by_name || null];
...
module.exports = {
  ...
  KINDS, VISIBILITY, DISPOSITIONS, VERIFICATION, REL_TYPES,
};
