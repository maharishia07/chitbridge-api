...
async function risk(entity_id) {
  ...
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      // total_value is NOT coalesced to 0 any more, and currency_code is selected. A null value is "not applicable"
      // — an offer nobody has agreed, or a non-monetary chit — and coalescing it to 0 turned both into zero-value
      // TRADES that diluted the concentration ratio this section exists to compute.
      `SELECT direction, sender_entity_display_name, all_recipients, total_value, currency_code, purpose, created_at
  ...
  // ── CONCENTRATION, SPLIT BY CURRENCY ────────────────────────────────────────────────────────────────────────
  // Athi, 2026-07-31: "any summary operation, clearly split based on currency… if the chit does not hold a currency,
  // that means it is information or helpdesk kind of activity."
  //
  // This summed every chit into one number regardless of denomination, so an entity trading in INR and AED got a
  // concentration ratio computed over INR + AED added together — a confident figure that means nothing. And every
  // help-desk ticket counted as a zero-value trade, inflating the denominator's row count while adding no value.
  //
  // Three states, kept apart:
  //   currency + value   a monetary claim → aggregated, within its own currency
  //   currency + null    monetary, not yet agreed (an offer) → counted, never valued
  //   NO currency        activity, not money → counted separately, never in a money total
  const nonMonetary = rows.filter((r) => !r.currency_code).length;
  const pending = rows.filter((r) => r.currency_code && r.total_value == null).length;
  const valued = rows.filter((r) => r.currency_code && r.total_value != null && Number.isFinite(Number(r.total_value)));

  const buckets = {};   // currency → { byParty, total, n }
  valued.forEach((row) => {
    const cur = String(row.currency_code).toUpperCase();
    const b = buckets[cur] || (buckets[cur] = { byParty: {}, total: 0, n: 0 });
    const v = Number(row.total_value); const cp = counterparty(row);
    b.byParty[cp] = (b.byParty[cp] || 0) + v; b.total += v; b.n++;
  });

  const byCurrency = Object.keys(buckets).map((cur) => {
    const b = buckets[cur];
    const ps = Object.keys(b.byParty).map((k) => ({ counterparty: k, value: round1(b.byParty[k]),
      pct: b.total ? round1((b.byParty[k] / b.total) * 100) : 0 })).sort((a, x) => x.value - a.value);
    return { currency: cur, total: round1(b.total), chits: b.n, top: ps[0] || null, parties: ps.slice(0, 8) };
  }).sort((a, b) => b.total - a.total);

  // Backward-compatible fields describe the LARGEST currency, so a single-currency entity — the overwhelming case —
  // sees exactly what it saw before. `by_currency` is where a multi-currency entity gets the truth.
  const primary = byCurrency[0] || null;
  const parties = primary ? primary.parties : [];
  const top = primary ? primary.top : null;
  ...
  if (top && top.pct >= flagPct) facts.exposures.push({ kind: 'concentration', severity: top.pct >= 60 ? 'high' : 'medium',
    // The currency is named whenever there is more than one, so the figure can never be read as a whole-book number.
    fact: round1(top.pct) + '% of your ' + (byCurrency.length > 1 ? primary.currency + ' ' : '') + 'business sits with one buyer (' + top.counterparty + ').',
  ...
  facts.concentration = { top: top || null, parties: parties.slice(0, 8), chits: n,
    ...
    // Stated, not hidden: how much activity is deliberately absent from the money view.
