...
function verifyFormat(id_type, id_value) {
  ...
  if (!v) return { ok: false, reason: 'empty', note: 'No value provided.' };
  if (!p) return { ok: true, id_type: key, value: v, label: key.toUpperCase(), unstructured: true };  // cert numbers etc. — no strict registry pattern
...
const PROVIDERS = {
  ...
  sandbox: {
    ...
    lookup: async (id_type, value) => {
      ...
      if (!res.ok) { let t = ''; try { t = await res.text(); } catch (_) {} throw new Error('sandbox ' + id_type + ' HTTP ' + res.status + (t ? ': ' + t.slice(0, 300) : '')); }
      ...
      // T2 (reviewer 2026-07-13) — NO fail-open defaults. `found` is POSITIVELY derived (a real registry record has a
      // legal name, or an explicit found flag); `active` is false (not undefined) when the status can't be mapped.
      const active = d.active != null ? !!d.active : (status ? /active|valid|registered|exist/i.test(String(status)) : false);
      const found  = d.found  != null ? !!d.found  : !!legal_name;
      return { found, active, legal_name, status, _raw: d };
    },
  },
  ...
  generic: {
    ...
    lookup: async (id_type, value) => {
      ...
      const legal_name = d.legal_name || d.name || d.trade_name || null;
      const active = d.active != null ? !!d.active : (status ? /active|valid|registered/i.test(String(status)) : false);   // T2 — no fail-open
      const found  = d.found  != null ? !!d.found  : !!legal_name;
      return { found, active, legal_name, status, _raw: d };
    },
  },
};
// PER-STANDARD SOURCE routing — each verifiable id_type points at the source that can confirm it. Sources marked (UAT)
// aren't implemented as providers yet; routing to them returns null → honest 'declared' until their adapter + key land in
// UAT (add a PROVIDERS entry, no other change). See VERIFY-SOURCES.md.
const PROVIDER_ROUTES = {
  gstn: 'sandbox',        // GST registry (Sandbox.co.in) — LIVE
  pan:  'sandbox',        // PAN (Sandbox KYC) — wire in UAT
  iso:  'iafcertsearch',  // ISO 9001/14001/… → IAF CertSearch (UAT)
  reach:'echa',           // REACH → ECHA registered substances (UAT)
  fssai:'foscos',         // FSSAI → FoSCoS licence search (UAT)
};
function getProviderFor(id_type) {
  const name = String(PROVIDER_ROUTES[String(id_type).toLowerCase()] || process.env.CB_KYB_PROVIDER || '').toLowerCase();
...
async function verifyRegistry(id_type, id_value) {
  ...
  const prov = getProviderFor(f.id_type || id_type);     // route to THIS id_type's source
  if (!prov) {
  ...
  // T2 (reviewer 2026-07-13) — FAIL CLOSED: nothing is verified unless the registry POSITIVELY confirms found AND active.
  // `undefined`/unmapped ⇒ NOT verified. An unparseable response (changed fields, error envelope, empty body) → unconfirmed.
  if (!r || r.found !== true || r.active !== true) {
    try { require('./logger').warn('verify unconfirmed', { id_type: f.id_type, provider: prov.name, shape: r && { found: r.found, active: r.active, status: r.status } }); } catch (_) {}
    return { ok: false, reason: 'unconfirmed', note: (f.label + ' was not positively confirmed by the registry (needs found + active).') };
  }
...
module.exports = { verifyRegistry, verifyFormat, getProviderFor, PROVIDER_ROUTES,
