...
async function _sbAuth() {
  ...
  const res = await fetch(base + '/authenticate', { method: 'POST', headers: {
    ...
    'x-api-version': process.env.SANDBOX_API_VERSION || '1.0', 'Content-Type': 'application/json' } });
...
const PROVIDERS = {
  ...
  sandbox: {
    ...
    lookup: async (id_type, value) => {
      ...
      if (id_type === 'gstn')      { url = base + (process.env.SANDBOX_GST_PATH || '/gst/compliance/public/gstin/' + value); }
      else if (id_type === 'pan')  { url = base + (process.env.SANDBOX_PAN_PATH || '/pans/' + value + '/verify'); method = 'POST'; body = JSON.stringify({ pan: value, consent: 'Y', reason: 'KYB verification' }); }
      else if (id_type === 'iec')  { url = base + (process.env.SANDBOX_IEC_PATH || '/dgft/public/iec/' + value); }
      ...
      const d = j.data || j.result || j;   // normalise (field names vary per endpoint — adjust here after seeing a real response)
      ...
      const legal_name = d.legal_name || d.legal_name_of_business || d.lgnm || d.name || d.trade_name || null;
