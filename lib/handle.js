 ...
 *
 * ⭐⭐ THE WHOLE NAMESPACE IS REGISTERED IN docs/NAMESPACE.md — every kind of id, what separates them, where
 * each is enforced, and what is specified but NOT built. This file holds four of those kinds and the rules
 * below are the authority for them; the register exists because the grammar used to live only here, in
 * comments, and could not be read by anyone who did not already know to open this file.
 * ⚠️ Add a kind here → add its row there, in the same commit.
 *
 ...
 */
const RESERVED_FIXED = ['api', 'www', 'app', 'admin', 'root', 'cb', 'chitbridge', 'network', 'system', 'support', 'help'];

/**
 * ── ⭐⭐⭐ AND THIS DEPLOYMENT'S OWN ROOT HANDLE, FROM THE ENVIRONMENT ─────────────────────────────────────────
 *
 * Athi, 2026-09-14: *"we reserve this name, and in the config or environment we have to use that name to
 * prevent being registered and should use it for this purpose."*
 *
 * The platform operator's entity — `chitbridgeinc`, `cbincroot`, `railmailroot` — must not be registrable by a
 * customer. ⚠️ The fixed list above does NOT cover it: it reserves the bare word `root`, not every handle that
 * ends in one, so `cbincroot` is free for anyone to take until it is named here.
 *
 * ⭐ IT COMES FROM THE ENVIRONMENT, NOT FROM THIS LIST, because a white-label deployment reserves a DIFFERENT
 * name. Hard-coding ours would mean every operator inherits a reservation for a brand that is not theirs and
 * lacks one for the brand that is. One variable, one deployment, one reserved root.
 *
 * ⚠️ RESERVING THE NAME IS NOT WHAT GRANTS THE AUTHORITY. This stops the handle being squatted; the entity is
 * still identified by its UUID (PLATFORM_ROOT_ENTITY, see lib/platformroot.js). Both are needed and neither
 * substitutes for the other — a name-based lookup would make anyone who registered the handle into the
 * operator, which is the hole this reservation closes rather than opens.
 */
const RESERVED_ROOT = String(process.env.PLATFORM_ROOT_HANDLE || '').trim().toLowerCase() || null;

const RESERVED = RESERVED_ROOT && RESERVED_FIXED.indexOf(RESERVED_ROOT) < 0
  ? RESERVED_FIXED.concat([RESERVED_ROOT])
  : RESERVED_FIXED;
...
/**
 * ── ⭐⭐⭐ THE FIFTH NAME: AN EMPLOYEE ID THAT IS STORED, NOT RENDERED ──────────────────────────────────────────
 *
 * Athi, 2026-09-15:
 *
 ...
 *
 * ⭐ THE SUFFIX IS THE POINT, AND SO IS THE STORING. Until now the employee handle was COMPOSED at display time
 * from `actor_key` + the parent's current `user_id`, and written nowhere. That made it unaddressable: nothing
 * could look one up, and `identities.user_id` held only entities. One column now holds all three kinds, each
 * saying which it is:
 *
 *     acmetraders                  an ENTITY
 *     ravi@acmetraders.br          an EMPLOYEE   ⭐ .br — "belongs to a business"
 *     9876512345@alpha-timers.cr   a CUSTOMER    (routes/catalogue.js crHandle, built since b170)
 *
 * ── ⚠️⚠️ IT IS THE SAME BUILDER PROBLEM `crHandle` ALREADY SOLVED, SO IT IS THE SAME SHAPE ────────────────────
 *
 * `crHandle`'s comment is the specification for this one, and it was written after the mistake:
 *
 *   · ONE builder, used at every call site, because a returning person must regenerate the SAME string. Two
 *     builders is two spellings of one identity and nobody notices until a login fails.
 *   · The handle is COMPARED, never PARSED. That is what makes the fallback safe below.
 *   · ⚠️ AND THE FALLBACK IS LOAD-BEARING, NOT PADDING. An entity registered before b170 has a NULL `user_id`,
 *     so its people fall back to the BRIDGE ID — `ravi@CBZQK5DAH9.br`. Ugly and unreadable, and the only value
 *     that is guaranteed non-null and stable. Both forms exist at once and that is fine.
 *
 * ⚠️ WHAT THIS DOES NOT DO: decide anything about login. A person types `ravi@acmetraders` and must keep
 * working; the suffix is what is STORED, not what is demanded at a keyboard. See routes/actors.js.
 */
const EMPLOYEE = '.br';
const CUSTOMER = '.cr';          /* built in routes/catalogue.js crHandle() — named here so the grammar is in one place */

/* ⚠️ THE BUILDER LIVES IN lib/mintuserid.js, NOT HERE. Athi, 2026-09-15: *"mint user id should be a module, and
   it should have these combinations in one place, similarly, resolve user id should be a module."* This file
   says what the shapes ARE and answers questions about them; making one is somebody else's job, and reading one
   is a third's. What stays here is the grammar the other two are written against. */

/** ⭐ is this an employee id? The suffix is unforgeable: no registered handle may contain "@" at all. */
function isEmployee(handle) {
  const h = String(handle == null ? '' : handle).trim().toLowerCase();
  return h.endsWith(EMPLOYEE) && h.indexOf('@') > 0;
}

/** ⭐ is this a storefront customer id? Same test, the other suffix. */
function isCustomer(handle) {
  const h = String(handle == null ? '' : handle).trim().toLowerCase();
  return h.endsWith(CUSTOMER) && h.indexOf('@') > 0;
}

/**
 * `ravi@acmetraders.br` → { actor_key: 'ravi', at: 'acmetraders' }, or null.
 * ⚠️ Split from the LAST "@": a customer id legitimately contains one on the left (`xyz=gmail.com@shop.cr`
 * swaps it, but nothing guarantees a future form will), and attributing a person to the wrong business is the
 * one mistake here that cannot be noticed by reading the row.
 */
function employeeParts(handle) {
  const h = String(handle == null ? '' : handle).trim().toLowerCase();
  if (!isEmployee(h)) return null;
  const body = h.slice(0, -EMPLOYEE.length);
  const cut = body.lastIndexOf('@');
  if (cut < 1 || cut === body.length - 1) return null;
  return { actor_key: body.slice(0, cut), at: body.slice(cut + 1) };
}
...
module.exports = { slug, check, checkRoot, child, rootOf, sameRoot, minted, isMinted, mintedParts, mintedPrefix,
                   isEmployee, isCustomer, employeeParts, EMPLOYEE, CUSTOMER,
