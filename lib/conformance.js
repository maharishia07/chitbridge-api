...
async function checkConformance(data, scope) {
  ...
  try {
    ...
    for (const s of r.rows) {
      const ref = versionref.format(s.standard_key, s.version);
...
async function captureFieldsForEntity(entity_id) {
  ...
  try {
    ...
    if (bk) { const bp = await resolveBoilerplate(bk); if (bp) keys = Object.values(bp.standards || {}).map((r) => versionref.keyOf(r)); }
