...
async function listChannels(entity_id) {
  ...
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      `SELECT id, channel, address, label, status, verified_at, verified_via, created_at
 ...
 */
async function approveBinding(id, via) {
  try {
    /* ⚠️ NO withEntity HERE, DELIBERATELY. The platform operator is not an entity and has no session to set; under
       FORCE RLS an entity-scoped update could not reach another entity's row. This is an admin action, gated by
       CB_ADMIN_KEY at the route, and it is the only write in this file that is not entity-scoped. */
    const r = await query(
      `UPDATE channel_binding SET status = 'verified', verified_at = now(), verified_via = $2, updated_at = now()
        WHERE id = $1 RETURNING id, entity_id, channel, address, status, verified_at, verified_via`,
      [id, via || 'platform']);
...
async function revokeBinding(id) {
  try {
    const r = await query(
      `UPDATE channel_binding SET status = 'declared', verified_at = NULL, verified_via = NULL, updated_at = now()
        WHERE id = $1 RETURNING id, entity_id, channel, address, status`, [id]);
    if (!r.rows.length) { const e = new Error('Binding not found'); e.status = 404; throw e; }
    return r.rows[0];
  } catch (e) { if (e && e.status) throw e; throw _missing(e); }
}
...
module.exports = { CHANNELS, listChannels, addBinding, approveBinding, revokeBinding, removeBinding, ownerOf, normalise, providerReady };
