...
async function listChannels(entity_id) {
  ...
  try {
    const r = await withEntity(entity_id, (c) => c.query(
      `SELECT id, channel, address, label, status, verified_at, verified_via, templates, created_at
...
module.exports = { CHANNELS, listChannels, listOutbound, setTemplate, addBinding, approveBinding, revokeBinding, removeBinding, ownerOf, normalise, providerReady };
