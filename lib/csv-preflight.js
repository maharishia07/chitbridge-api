...
const SYNONYMS = {
  /**
   * ⚠️⚠️ HSN IS NOT A SKU, AND MAPPING IT THERE WAS A CATEGORY ERROR WITH TEETH. An HSN code classifies a KIND
   * of goods for tax — "Tea, 250g" and "Tea, 1kg" are both 09024090 — while a SKU identifies ONE product.
   * Found on a real wholesaler's price list (tests/fixtures/pricelist-wholesaler.csv): the file mapped its HSN
   * column onto the identity key, and preflight then reported two perfectly good rows as DUPLICATES because they
   * shared a tax code. Worse than the false error: had it gone through, the catalogue's identity would BE the
   * tax code, so every later upload would collapse a whole product family onto one row.
   *
   * HSN is now its own canonical — which is also what lib/tax.js needs per line.
   */
  sku:            ['sku', 'code', 'item code', 'itemcode', 'product code', 'article', 'article no', 'part no', 'part number',
                   'mpn', 'barcode', 'ean', 'gtin', 'upc', 'item id', 'id'],
  hsn:            ['hsn', 'hsn code', 'hsn sac', 'hsn/sac', 'sac', 'sac code', 'tariff', 'tariff code', 'commodity code'],
  gst_rate:       ['gst rate', 'gst', 'gst %', 'gst percent', 'tax rate', 'tax %', 'rate of tax', 'igst rate'],
  ...
  /**
   * ⚠️⚠️ MRP IS NOT A SELLING PRICE, AND FOR A WHOLESALER THAT DIFFERENCE IS THE WHOLE MARGIN. Maximum Retail
   * Price is the ceiling printed on the pack — what a CONSUMER may be charged. A trade price list carries both:
   * "MRP 220" beside "Rate 180". With mrp listed here, MRP WON the match on the real fixture and "Rate (INR)"
   * was reported as the conflicting duplicate — so a wholesaler importing their own price list would have
   * published RETAIL prices to their trade customers, on every line, silently.
   *
   * ⭐ MRP is now its own canonical. An unmatched MRP column is one question a merchant answers once; a wrongly
   * matched one is margin lost on every order until somebody notices.
   */
  /**
   * ⚠️⚠️⚠️ 'cost' WAS HERE, AND IT COST A SHOP ITS OWN SELLING PRICE. Athi, live, testing Offer Lab: "the
   * cost price is not storing." It was worse than not storing — fold() (lib/catalogue-columns.js) applies
   * THIS SAME synonym table to every catalogue write, not only a CSV import, so a caller setting
   * item_data.cost had it silently renamed onto item_data.price. A plain number slipped past validation and
   * overwrote the real selling price outright (reproduced live, then reverted); Offer Lab's own richer
   * `{value, source, updated_at}` cost shape failed price's number check and came back "Price must be a
   * number" — a sentence about the wrong field, for a person who never touched price.
   *
   * The rest of this codebase already treats cost and price as two DELIBERATELY separate, un-mergeable
   * concepts — lib/item-cost.js: "must never reach a customer-facing surface, no seller switch"; the whole
   * [OFFR-01]–[OFFR-04] cost-visibility gate exists only because they are not the same number. 'price' as a
   * synonym for a supplier's own trade/wholesale price sheet is a real, separate use — kept below — but
   * 'cost' must never fold onto it, exactly the MRP lesson two entries above: "a wrongly matched one is
   * margin lost … until somebody notices." An incoming 'cost' column, CSV or API, now declares its own
   * column instead — which is what every other cost-aware surface in this codebase already assumes exists.
   */
  price:          ['price', 'rate', 'amount', 'unit price', 'unit rate', 'selling price', 'sale price',
                   'list price', 'price per unit', 'value', 'trade price', 'wholesale price', 'net rate'],
  mrp:            ['mrp', 'max retail price', 'maximum retail price', 'retail price', 'printed price'],
  ...
  /* Renamed with the sheet (lib/sheet.js): a merchant now reads and writes `available` and `qty`, so the
     synonyms must land on those, or a real file maps onto a column the template no longer offers. */
  available:      ['availability', 'available', 'status', 'stock status', 'in stock', 'is available'],
  qty:            ['qty', 'available qty', 'available quantity', 'quantity available', 'stock', 'stock qty', 'on hand',
  ...
  /**
   * ⚠️⚠️ ADDED [TILL-107], 2026-09-19 — AND IT WAS MISSING ALTOGETHER. "Group" scored 0.33 and was dropped,
   * which is Tally's own word: a stock item's PARENT is its stock group, and tools/tally-connector fetches it
   * by that name. So a catalogue exported from the accounting package every Indian shop already runs arrived
   * here with its grouping silently discarded.
   * ⚠️ This is the field that groups the KEYS on the counter. Without it a shop gets one flat wall of
   * products, which is the till a busy shop cannot work.
   * ⚠️ 'menu' and 'menu section' are here for the hotel trade, where that is simply what the column is called.
   * ⚠️ NOT 'type' or 'class' — both are far too greedy ('pack type', 'class of goods') and would steal
   * headers that belong elsewhere. A synonym that is usually wrong is worse than a missing one, because the
   * import looks like it worked.
   */
  category:       ['category', 'categories', 'group', 'stock group', 'item group', 'product group',
                   'section', 'department', 'dept', 'menu', 'menu section', 'family'],
};
...
function matchHeader(header, accepted, labels) {
  ...
  /**
   * ⚠️⚠️ 'YOUR CATALOGUE DOES NOT USE THIS' IS TWO DIFFERENT ANSWERS, AND SAYING THE HARSH ONE TO BOTH IS WHAT
   * MAKES A REAL PRICE LIST FEEL UNIMPORTABLE.
   *
   * On tests/fixtures/pricelist-wholesaler.csv — an ordinary 12-line wholesaler sheet — HSN Code, MRP and GST %
   * all came back "which your catalogue does not use". Every one of those is a perfectly ordinary column that
   * this catalogue simply had not declared YET, and since the declare-first writer landed, adding one is a tick.
   * Telling a merchant their own price list is unusable, when the honest answer is "shall I add that column?",
   * is the difference between adopting the product in ten minutes and giving up.
   *
   * ⭐ ONLY A MODE CONFLICT IS A REAL REFUSAL. `price_min` in a fixed-price shop, or `price` in a catalogue that
   * takes no prices, contradicts a declaration the merchant has already made — those must be refused, because
   * importing them would produce rows the shop cannot sell. Everything else is a column they do not have yet.
   */
  const MODE_KEYS = ['price', 'price_min', 'price_max'];
  for (const [canon, alts] of Object.entries(SYNONYMS)) {
    if (accepted.includes(canon)) continue;
    if (canon !== n && !alts.some((a) => normalise(a) === n)) continue;
    if (MODE_KEYS.indexOf(canon) >= 0) {
      return { canonical: null, how: 'not-accepted', confidence: 1,
        why: `this looks like ${canon}, which contradicts how your catalogue takes orders — a ${accepted.includes('price') ? 'fixed-price' : 'price-free'} catalogue has no ${canon} column` };
    }
    return { canonical: null, how: 'addable', confidence: 1, suggest: canon,
      why: `this is ${canon}, which your catalogue does not have yet — map it to add it as a new column` };
  }
...
const NUMERIC_FIELDS = ['price', 'price_min', 'price_max', 'qty', 'available_qty', 'lead_time_days', 'min_order_qty'];

/**
 * ⚠️⚠️ AN IDENTIFIER IS NEVER A NUMBER, HOWEVER MUCH IT LOOKS LIKE ONE — and this was found by importing a real
 * wholesaler's price list, not by reasoning about it.
 *
 * `inferType` reads a column's VALUES, which is right, and every HSN code in the file was digits — so it declared
 * `hsn` a number, and `looseNumber` turned **09024090 into 9024090**. An HSN code with its leading zero stripped
 * is not a slightly-off number, it is an INVALID CODE: chapters 01–09 cover live animals, vegetables, coffee,
 * tea and spices, so the entire Indian grocery trade is in the range that breaks. The same trap sits under sku,
 * barcode, EAN, PIN and GSTIN.
 *
 * ⭐ The rule is about the FIELD, not the values: these identify, they do not measure. Nothing is ever added to
 * an HSN code, so nothing is lost by keeping it text — and a leading zero is kept.
 */
const IDENTIFIER_FIELDS = ['sku', 'hsn', 'code', 'item_code', 'barcode', 'ean', 'gtin', 'upc', 'mpn',
  'part_no', 'part_number', 'article', 'pin', 'pincode', 'gstin', 'batch_no', 'serial', 'serial_no'];
...
function looseNumber(raw) {
  ...
  const stripped = s
    ...
    /**
     * ⚠️⚠️ THE CURRENCY WORD, BY NAME ([TILL-108]). "Rs 30" is how a great many Indian sheets write a
     * price, and it came back null — so a file with one such cell was refused entirely, because the import is
     * all-or-nothing by design. A shop cannot open while its own spreadsheet is unreadable.
     * ⚠️ A NAMED LIST, NEVER \b[A-Za-z]+\b. Stripping any word would read "30 kg" as 30 — a QUANTITY taken
     * for a price, silently. A refusal a person can act on beats a number nobody checked.
     */
    .replace(/\b(?:rs|rs\.|inr|aed|dhs|dh|usd|eur|gbp)\b\.?/gi, '')
    .replace(/\/-\s*$/, '')                // "30/-", the way an Indian invoice ends a figure
    .replace(/[,\s]/g, '')
    .trim();
...
function preflight(opts = {}) {
  ...
  /**
   * ⭐⭐ MRP BECOMES THE PRICE ONLY WHEN NOTHING ELSE DID — and this conditional is the whole of a decision that
   * was previously made one way and was wrong the other way round.
   *
   * A small retailer's sheet has ONE money column headed MRP, and they do sell at it: refusing to map it would
   * make them answer a question with one possible answer. A WHOLESALER's sheet has BOTH — "MRP 220" beside
   * "Rate 180" — and there MRP is a printed ceiling, not their trade price. The old synonym list had `mrp` on
   * `price` unconditionally, so on the real fixture MRP won and "Rate (INR)" was reported as the redundant
   * duplicate. That wholesaler would have published RETAIL prices to trade customers on every line, silently.
   *
   * ⚠️ Neither 'always' nor 'never' is true. The file itself says which case it is: if some other column already
   * claimed `price`, MRP is a ceiling and keeps its own column. If none did, MRP is what they sell at.
   */
  if (accepted.includes('price') && !claimed.has('price')) {
    /* ⚠️ MRP reaches here in one of three shapes: matched to `mrp` (when the catalogue declares that column),
       'not-accepted' (it does not), or 'conflict' (another column already took price). All three are the same
       header, so all three are recognised — matching on the NAME, not on how the matcher happened to file it. */
    const isMrp = (x) => SYNONYMS.mrp.indexOf(normalise(x.incoming)) >= 0;
    const m = mapping.find((x) => x.canonical === 'mrp' || (!x.canonical && isMrp(x)) || (x.how === 'conflict' && isMrp(x)));
    if (m) {
      m.canonical = 'price';
      m.how = 'synonym';
      m.confidence = 0.9;
      m.why = 'this is the only price column in the file, so it is being read as your selling price — '
            + 'if you sell below MRP, map your trade price instead';
      claimed.set('price', m);
    }
  }
  ...
  // The identity key, DECLARED. Hardcoding `sku` was an assumption: pharma identifies a lot by batch_no and a
  // commodity desk may need hs_code + origin_country. resolve() falls back to ['sku'], so an undeclared catalogue
  // behaves exactly as it did.
  const idKey = (opts.identity && Array.isArray(opts.identity.key) && opts.identity.key.length)
    ? opts.identity.key : ['sku'];
  ...
  rows.forEach((r, n) => {
    ...
    if (idKey.every((k) => mapped.includes(k))) {
      const parts = idKey.map((k) => String(cellAt(r, k) == null ? '' : cellAt(r, k)).trim());
      if (parts.every(Boolean)) {                       // a PARTIAL identity is not an identity
        const ident = parts.join(' ');
        if (seenIdentity.has(ident)) {
          add(line, idKey[0], `"${parts.join(' + ')}" also appears on line ${seenIdentity.get(ident)}`, 'error');
        } else seenIdentity.set(ident, line);
      }
    }
  });
  ...
  const missingId = idKey.filter((k) => accepted.includes(k) && !mapped.includes(k));
  if (missingId.length && rows.length) {
    notes.push(`⚠ No ${missingId.join(' / ')} column. Products are matched by ${idKey.join(' + ')}, so every row here will be ADDED as a new product — nothing can be updated, and uploading this file twice would create duplicates.`);
  }
...
function applyDecisions(opts = {}) {
  ...
  for (const d of decisions) {
    ...
    if (action === 'create') {
      ...
      newFields.push({ field_key: key, field_name: String(incoming).trim().slice(0, 120),
        /* ⚠️ An identifier is text even when every value is digits — see IDENTIFIER_FIELDS. */
      field_type: IDENTIFIER_FIELDS.indexOf(key) >= 0
        ? 'text' : inferType(rows.map((r) => r[headers.indexOf(incoming)])), required: false });
      continue;
    }
  ...
  rows.forEach((r, n) => {
    ...
    for (const [, { field, idx }] of byHeader) {
      const raw = r[idx];
      if (raw === undefined || String(raw).trim() === '') continue;
      const numeric = IDENTIFIER_FIELDS.indexOf(field) < 0
        && (NUMERIC_FIELDS.includes(field) || (newFields.find((f) => f.field_key === field) || {}).field_type === 'number');
      if (numeric) {
    ...
    const idKeys = (opts.identity && Array.isArray(opts.identity.key) && opts.identity.key.length)
      ? opts.identity.key : ['sku'];
    const identified = idKeys.every((k) => String(it[k] == null ? '' : it[k]).trim());
    if (!String(it.name || '').trim() && !identified) return;
    items.push(it);
    lines.push(line);
  });
...
module.exports = { preflight, matchHeader, normalise, similarity, looseNumber, applyDecisions, toFieldKey, inferType,
  SYNONYMS, BLOCKED, QUALIFIERS, FUZZY_ACCEPT, NUMERIC_FIELDS, IDENTIFIER_FIELDS };
