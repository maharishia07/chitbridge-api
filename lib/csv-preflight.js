...
function matchHeader(header, accepted) {
  ...
  for (const k of accepted) {
    for (const alias of [k, ...(SYNONYMS[k] || [])]) {
...
function preflight(opts = {}) {
  ...
  const mapping = headers.map((h) => Object.assign({ incoming: h }, matchHeader(h, accepted)));
  ...
  const required = accepted.filter((c) => !optional.includes(c) && c !== 'sku');
...
module.exports = { preflight, matchHeader, normalise, similarity, looseNumber, SYNONYMS, BLOCKED, FUZZY_ACCEPT };
