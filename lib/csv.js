...
function columnsFor(items, schema) {
  ...
  // Anything the items carry beyond the schema, ALPHABETICAL — same reason as acceptedKeys below: these keys come
  // out of a jsonb column, whose order is Postgres's (by key length, then bytewise) and not the merchant's. Reading
  // them "in the order they appear" made an export's columns shuffle between one download and the next.
  const rest = [];
  for (const it of items) for (const k of Object.keys(it || {})) {
    if (k && !cols.includes(k) && !k.startsWith('_') && !rest.includes(k)) rest.push(k);
  }
  rest.sort();
...
function toItems(parsed) {
  ...
  return rows.map((r) => {
    ...
    for (const [k, raw] of Object.entries(r)) {
      ...
      // GOVERNANCE NEVER ARRIVES IN A CELL. Gamma's live export carried
      // `order_input` = `{"preset":"range","pipeline":"commerce"}` as a plain column, so a bulk import would have
      // read a MODE out of a spreadsheet — the exact thing the template deliberately refuses to offer. It is
      // exported (so nothing is lost when a catalogue is read out) and dropped here on the way back in.
      if (k === 'order_input') continue;
 ...
 */
/**
 * ⚠️⚠️ `availability` AND `available_qty` WERE OFFERED AND NEVER READ. No route in the codebase writes either;
 * they were placeholder columns whose own template note called them *"your own notes, not stock control"* — while
 * the real availability sat in `status` (an enum) and `avail` ({qty, source, as_of}) and leaked into the export
 * as JSON. Three names for one concept, two of them inert, and the working one unreadable.
 *
 * ⭐ Now the template offers the same two columns the export emits and the import understands — `available`
 * (yes/no) and `qty` — so what you download, what you edit and what you upload are the same words. See
 * lib/sheet.js. `qty_as_of` and `qty_source` are emitted by the EXPORT but never asked for by the template: they
 * are the system's answer, not the merchant's.
 */
const TRADE_EXTRAS = ['available', 'qty', 'lead_time_days', 'min_order_qty'];

/**
 * Keys a template must never ask for, whatever the catalogue carries.
 *
 * `quantity` is the CUSTOMER's, set at order time. `order_input` is the item's DECLARED MODE — see the note above on
 * why governance cannot travel in a cell; a sheet offering it would let someone type `{"preset":"range"}` into a cart
 * shop and be refused at order time.
 */
const RESERVED = ['quantity', 'order_input', 'is_active', 'item_id', 'entity_id', 'schema_id'];

/**
 * The columns this catalogue ACTUALLY uses.
 *
 * Found live, not in tests: Gamma's export carried `code` and `desc` while its template asked for neither, because
 * the template read only `schema_fields` and Gamma has no active default schema — its products carry those keys
 * directly. A merchant would have downloaded the sheet, filled it, uploaded it, and silently lost two columns.
 *
 * So the accepted format is the UNION of what the schema declares and what the items in the catalogue already carry.
 * That is what Athi meant by "its own catalogue style and the accepted format" — the format a shop actually uses,
 * not the one it happens to have registered.
 */
function acceptedKeys(schema, observed) {
  const out = [];
  const ok = (k) => k && !out.includes(k) && !k.startsWith('_')
    && !RESERVED.includes(k) && !MONEY_KEYS.includes(k)
    && !MONEY_KEYS.some((m) => k === m + '_currency')   // the server stamps it; never asked for
    && !TRADE_EXTRAS.includes(k);                        // always last, in a fixed order — see below

  // 1. The DECLARATION, in the order the merchant arranged it. This is the meaningful order.
  for (const k of Object.keys((schema && schema.properties) || {})) if (ok(k)) out.push(k);

  // 2. Keys the items carry but the schema does not — ALPHABETICAL, deliberately.
  //
  // ⚠️ These come back from a `jsonb` column, and Postgres does not preserve key order in jsonb: it normalises by
  //    key length, then bytewise. So reading them in "the order they appear" meant the template's columns were
  //    ordered by how Postgres happens to store them. Live proof: after one import the sheet came back
  //    `… available_qty, min_order_qty, lead_time_days …` — lengths 13, 13, 14 — which is jsonb's order, not the
  //    merchant's. A person's own template changing shape after every upload is not a cosmetic problem; it is the
  //    thing this feature exists to prevent.
  const rest = (Array.isArray(observed) ? observed : []).filter(ok);
  rest.sort();
  for (const k of rest) if (ok(k)) out.push(k);
  return out;
}
...
function templateFor(opts = {}) {
  ...
  for (const k of acceptedKeys(schema, opts.observed)) if (!cols.includes(k)) cols.push(k);
  ...
  //
  // ⚠️ EVERY column gets a value, and the row says what it is.
  //
  // The first version filled only the columns it had a nice example for and left the rest blank — so a merchant who
  // added their own column, uploaded it successfully, then downloaded the template again saw their column sitting
  // EMPTY beside populated ones, and reasonably concluded the value had been dropped. It had not: the data was
  // stored and exports fine. The template simply looked enough like an export to be mistaken for one.
  //
  // A template that can be misread as your data is a bad template, so this one announces itself.
  const eg = {};
  for (const c of cols) {
    if (c === 'sku') eg[c] = 'SKU-001';
    else if (c === 'name') eg[c] = 'EXAMPLE ROW — delete this line';
    ...
    else if (c === 'available') eg[c] = 'yes';
    else if (c === 'qty') eg[c] = 25;
    else if (c === 'lead_time_days') eg[c] = 7;
    else if (c === 'min_order_qty') eg[c] = 1;
    else eg[c] = 'your value here';            // never blank — a blank cell reads as "this one did not work"
  }
  ...
  return {
    ...
    notes: [
      ...
      ...(usedExtras.length ? [
        ...
        `available is yes or no. qty is a plain number, or blank if you do not count stock — blank means "not said", which is not the same as zero.`,
        `When you upload, the date and the source are recorded for you. Uploading the same answer again refreshes that date, which is the point: it says the answer is still true today.`,
        `⚠ qty is RECORDED ONLY — it is not reduced when an order is placed and an order is not refused at zero. Treat it as your own count, not as stock control.`,
      ] : []),
