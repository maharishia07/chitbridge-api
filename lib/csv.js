...
function acceptedKeys(schema, observed) {
  const out = [];
  const push = (k) => {
    if (!k || out.includes(k) || k.startsWith('_')) return;
    if (RESERVED.includes(k) || MONEY_KEYS.includes(k)) return;
    if (MONEY_KEYS.some((m) => k === m + '_currency')) return;   // the server stamps it; never asked for
    out.push(k);
  };
  for (const k of Object.keys((schema && schema.properties) || {})) push(k);
  for (const k of (Array.isArray(observed) ? observed : [])) push(k);
  return out;
}
