...
async function partiesFor(hdr, me) {
  ...
  const recips = Array.isArray(hdr.all_recipients) ? hdr.all_recipients : [];
  const otherId = sent
    ? ((recips.find((r) => String(r.role || r.kind || 'to').toLowerCase() === 'to' || String(r.role || '') === 'Act') || recips[0] || {}).entity_id)
    : hdr.sender_entity_id;
  ...
  const seller = T.partyOf(sent ? meRow : otherRow, { entity_id: sent ? String(me) : String(otherId || '') });
  const buyer = T.partyOf(sent ? otherRow : meRow, { entity_id: sent ? String(otherId || '') : String(me) });
  return { seller, buyer, direction: sent ? 'sent' : 'received', me: T.partyOf(meRow, { entity_id: String(me) }) };
}
...
async function entryFor(hdr, me) {
  ...
  return { chit_id: hdr.chit_id, subject: hdr.manual_subject || hdr.auto_subject || '', purpose: hdr.purpose, status: hdr.current_status,
           direction: p.direction, at, seller: p.seller, buyer: p.buyer, invoice: inv.invoice, rated: inv.rated, unrated: inv.unrated,
