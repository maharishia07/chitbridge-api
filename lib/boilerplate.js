...
async function resolveBoilerplate(key, version) {
  ...
  try {
    ...
    const row = bp.rows[0];
    if (!row) return null;
    // enrich each declared source with its owning SOURCE-ENTITY (stable id + mutable name), joined via the content table
    const src = await query(
      `SELECT bs.source_key, bs.source_kind, bs.facet,
              i.identity_id AS source_entity_id, i.bridge_id AS source_bridge, i.display_name AS source_name
         FROM boilerplate_sources bs
         LEFT JOIN standard_source  ss ON ss.standard_key = bs.source_key
         LEFT JOIN catalogue_source cs ON cs.source_key  = bs.source_key
         LEFT JOIN identities        i ON i.identity_id  = COALESCE(ss.owner_entity_id, cs.owner_entity_id)
        WHERE bs.boilerplate_key = $1 AND bs.version = $2`,
