...
/* ⚠️ NAMED `uom`, NOT `units` — there is a local `const units = Object.keys(b.by_unit)` inside the resolution
   map below, and a module shadowed by a loop variable is the kind of bug that only shows up as a wrong number. */
const uom = require('./units');
...
function consolidate(requests, cat) {
  ...
  for (const req of (requests || [])) {
    ...
    for (const ln of (req.lines || [])) {
      ...
      /* ⚠️ normUnit, NOT norm — a UNIT is folded onto its canonical spelling here (கிலோ → kg), which `norm`
         cannot do because it also folds item NAMES and must never carry unit vocabulary. See lib/units.js. */
      const u = uom.normUnit(ln.unit || m.item.unit || '');
  ...
  const lines = [...buckets.values()].map((b) => {
    ...
    const canon = uom.normUnit(b.canonical_unit);
    /* ⚠️ STILL `norm` HERE — these are an item NAME and a VARIANT, not units. Unit vocabulary must never touch a
       product name; that is exactly why the aliases live in their own module instead of inside norm(). */
    const item = cat.items.find((x) => x.key === norm(b.item) + '|' + norm(b.variant || ''));
    const conv = (item && item.conversions) || {};
    /**
     * ⚠️ CONVERSION KEYS ARE WRITTEN BY THE ENTITY, so they are spelled however that entity spells them. Now
     * that `u` arrives folded (kilos → kg), an exact lookup could miss a conversion that used to resolve.
     * The exact key is tried FIRST and only a miss falls through to the folded comparison — so this can only
     * ever make more declared conversions resolve, never fewer, and no existing successful lookup changes.
     * ⚠️ It does NOT invent conversions: a factor must still have been DECLARED on the item. Folding only
     * decides which declared factor a written unit refers to.
     */
    const convFactor = (u) => {
      if (conv[u] !== undefined) return Number(conv[u]);
      const k = Object.keys(conv).find((x) => uom.normUnit(x) === u);
      return k === undefined ? NaN : Number(conv[k]);
    };

    let total = 0, converted = [], unresolved = [];
    for (const u of units) {
      if (!canon || u === canon) { total += b.by_unit[u]; continue; }
      const factor = convFactor(u);
