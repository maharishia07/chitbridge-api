...
async function resolveReadiness(entity_id) {
  ...
  if (!docs.length) {   // FLOOR: never empty while active standards exist
    try { const g = await query(`SELECT DISTINCT standard_key FROM standard_source WHERE active = true`); const gd = await docsFor(g.rows.map(x => x.standard_key)); if (gd.length) { docs = gd; keys = g.rows.map(x => x.standard_key); } } catch (_) {}
  }
...
async function activeStandards() {
  try { const r = await query(`SELECT DISTINCT ON (standard_key) standard_key, title, applicability, guidance FROM standard_source WHERE active = true ORDER BY standard_key, minted_at DESC`); return r.rows; } catch (_) { return []; }
}
...
async function gatherDocument(entity_id, { standard_key, doc_key, evidence_ref, valid_until, status, verification }) {
    ...
    `INSERT INTO entity_compliance (entity_id, standard_key, doc_key, status, evidence_ref, valid_until, verification)
     ...
     ON CONFLICT (entity_id, standard_key, doc_key)
       DO UPDATE SET status = EXCLUDED.status, evidence_ref = EXCLUDED.evidence_ref, valid_until = EXCLUDED.valid_until,
         ...
         gathered_at = now()`,
    [entity_id, standard_key, doc_key, status || 'gathered', evidence_ref || null, valid_until || null, JSON.stringify(verification || {})]));
}
