 ...
 */
async function current(entity_id, chit_id, _db) {
  try {
...
async function byPerson(entity_id, opts = {}) {
  ...
  try {
    const r = await withEntity(entity_id, (db) => db.query(
      `WITH latest AS (
       ...
       SELECT a.assignee_actor_id, a.assignee_name, a.assignee_type, a.task, a.due_date, a.note,
         ...
         LEFT JOIN chit_header h ON h.entity_id = $1 AND h.chit_id = a.chit_id
    ...
    return { people: out, lines: r.rows.length, migrated: true };
