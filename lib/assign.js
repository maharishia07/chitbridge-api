    ...
    const r = await withEntity(entity_id, (db) => db.query(
      `WITH latest AS (
       ...
       SELECT a.assignee_actor_id, a.assignee_name, a.assignee_type, a.task, a.due_date::text AS due_date, a.note,
          ...
          */
         LEFT JOIN LATERAL (
           SELECT manual_subject, auto_subject, sender_entity_display_name, is_draft
             FROM chit_header
        ...
        WHERE l.entity_id = $1
           ...
           * unassigned work somebody owes. Excluded here, not by status: a draft's status is not one of
           * the three closed ones.
           */
          AND COALESCE(h.is_draft, false) = false
