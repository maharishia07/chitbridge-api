  ...
  try {
    ...
    if (AUD === 'them') {
      ...
      const b = await query(
        ...
        [bid]);
      if (!b.rows[0]) return { ok: false, why: 'no active business with bridge id ' + bid };
...
async function preview(from, t) {
  ...
  if (!root) return { ok: false, why: 'no operator entity is configured (PLATFORM_ROOT_ENTITY)' };
