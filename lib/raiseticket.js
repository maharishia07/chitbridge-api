...
async function resolveDesk(from, t, root) {
  try {
    ...
    let desk = AUD === 'here' ? from.entity_id : root;
    if (AUD === 'them') {
    ...
    return { ok: true, audience: AUD, desk, to, op, route, isSelf, why: route.why };
  } catch (e) {
...
async function preview(from, t) {
  ...
  return { ok: true, audience: r.audience, to_name: r.op.display_name || null,
