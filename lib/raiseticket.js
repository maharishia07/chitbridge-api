 ...
 * @param t     { kind, ref, subject, detail, severity, screen_code, definition_id }
...
async function raise(from, who, t) {
  ...
  if (String(from.entity_id) === String(root)) return { raised: false, why: 'raised by the operator' };

  try {
    const them = await query(
      'SELECT bridge_id, display_name FROM identities WHERE identity_id = $1', [root]);
    const op = them.rows[0];
    if (!op) return { raised: false, why: 'operator entity not found' };

    /* ⭐ where it lands, and who owns it — b250. Resolved on the OPERATOR's side: the shop's routing is about
       the shop's own staff and has nothing to say about who answers a ticket here.
       ⚠️ BEFORE the transaction: routeFor opens its own withEntity, and nesting two pins two connections. */
    const route = await require('./workroute').routeFor(root, t.kind || 'incident');
    ...
    const all_recipients = [
      ...
      { entity_id: root, bridge_id: op.bridge_id, display_name: op.display_name, role: 'receiver' },
    ];
    ...
    const copies = [
      ...
      mint.party(headerCommon, { entity_id: root, direction: 'received', role: 'Act',
        ...
        log: { action: 'delivered', action_by_identity_id: from.entity_id,
               ...
               detail: 'Support ticket from ' + (from.display_name || 'a shop') } }),
    ];
    ...
    let folderName = null;
    try {
      await withEntity(root, async (db) => {
        ...
        if (!fid) {
          ...
          if (!fid) {
            const ins = await db.query(
              'INSERT INTO folder (entity_id, name) VALUES ($1,$2) RETURNING folder_id', [root, DEFAULT_FOLDER]);
            fid = ins.rows[0].folder_id;
          }
        }
        const nm = await db.query('SELECT name FROM folder WHERE folder_id = $1 AND entity_id = $2', [fid, root]);
        ...
        if (route.assignee_actor_id) {
          ...
          if (a.rows[0]) {
            await db.query(
              ...
              [route.assignee_actor_id, a.rows[0].display_name, chit_id, root]);
          }
        }
      });
    } catch (e) {
    ...
    return { raised: true, chit_id, folder: folderName, assigned: !!route.assignee_actor_id, routed: route.why };
  } catch (e) {
