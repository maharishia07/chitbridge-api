...
| 5 | `png/HubFooter.png` | The CAREFUL footer: why it fails, where the three actions belong, and the confirm sheet that replaces it — spec §9. |
| 6 | `png/HubStartPhone.png` | Phone. **Write this layout first**, then widen. |
| 7 | `source/Hub*.dc.html` | Exact colours, sizes, spacing and copy. **Reference only — never ship these.** They use a design tool's template format (`x-dc`, `sc-for`, `{{holes}}`), not app code. |
...
## 5. Dissolve the CAREFUL footer (spec §9)

A pinned red strip is a warning nobody reads by day two, and its three actions share nothing but the warning. **Sign out** goes to the person block in the header; **Close this counter** becomes **Close the day** on the day card; **Clear and reload** becomes **Repair this counter** in Counter health ▸ technical. The warning moves to a confirm sheet at the moment of the act, stating the consequence and the live numbers (*96 of 96 sent · none waiting · none in hand*), with **Not now** as the plain way out.
...
```
Read EVERY file in design-handoff/counter-hub/ first — every .md and every .png — before writing any code.
