...
- **Modularity (DONE)**: co-assist screen/render/actions now load lazily from **`cap-workforce.js`** (`CAP_OF.coassists='workforce'`); help/assistant CONTENT (`CO_HELP`/`ASSIST_LIB`/`COMPOSE_HELP`) split into lazy **`cap-help.js`**, gated behind first `openHelp`/`openAssist` — so only functionality ships at co-assist runtime, help downloads on demand. Shared helpers (`mapApiActor`, `hatLabel`, `hatAssignable`, `HAT_LABEL`, `acType`, the modals) stay eager in Core. See `COASSIST-REGRESSION.md` §Modularity.
