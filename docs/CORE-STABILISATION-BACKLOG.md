...
| **G-demo-1** | Demo fidelity: drop network per-node grant overclaim; mark /api/network preview | P1 | **DONE** web `2fbb2cc` | — | Per-node `cap` grant + "grant flows down, default-deny" removed; /api/network badged preview/editing-not-live. |
| **G-demo-2** | Demo fidelity: actors show free-text role labels, no implied enforced view-only/audit | P1 | **DONE** web `2fbb2cc` | — | Caps now Yard/Office/Site survey; co-assist help honest-first (per-actor engagement types PLANNED). Per-actor capability shown only when D1 ships. |
| **G-demo-3** | Demo fidelity: **"view as counterparty" perspective switch** (+ second-entity blob) to PROVE isolation + two-party message visibility | P1 | **DESIGN** (`DEMO-PERSPECTIVE-SWITCH.md`) | — | The highest-value demo upgrade for proving the security/multitenancy story. |
